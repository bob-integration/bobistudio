***REMOVED*** Flash du firmware Broadcom de r620-1 (nœud 42) — procédure

**Date** : 2026-08-07 · **Opérateur** : à faire à la main · **Machine** : r620-1, `192.168.1.3`

***REMOVED******REMOVED*** Pourquoi

r620-1 n'établit **aucun** lien RDMA depuis son enrôlement (lien ***REMOVED***92 créé le 2026-08-03, jamais
...
***REMOVED******REMOVED*** Impact et filet
...
- Les deux liens RDMA en attente (***REMOVED***92, ***REMOVED***113) ne transportent rien aujourd'hui : ils échouent déjà.

***REMOVED******REMOVED*** Étape 0 — le coup gratuit : cycle d'alimentation À FROID
...
***REMOVED******REMOVED*** Étape 1 — récupérer le paquet firmware
...
***REMOVED******REMOVED*** Étape 2 — flasher

***REMOVED******REMOVED******REMOVED*** Voie A (à essayer d'abord) — l'exécutable Dell

```bash
chmod +x Network_Firmware_2WPNV_LN64_23.11.16.22.BIN
./Network_Firmware_2WPNV_LN64_23.11.16.22.BIN            ***REMOVED*** vérifie la compatibilité, puis flashe
```
...
***REMOVED******REMOVED******REMOVED*** Voie B — extraire le `.pkg` et passer par devlink
...
find /root/fw -name '*.pkg'                     ***REMOVED*** typiquement BCM5741x…pkg
...
***REMOVED******REMOVED*** Étape 3 — cycle d'alimentation à froid, obligatoire
...
***REMOVED******REMOVED*** Étape 4 — vérification

Sur le nœud :

```bash
devlink dev info pci/0000:41:00.0 | grep -A3 running     ***REMOVED*** doit afficher fw 23.11.16.22
dmesg | grep -c 'Failed to create HW CQ'                 ***REMOVED*** doit rester à 0 après plusieurs minutes
```

Attendu : `fw 23.11.16.22`, `fw.mgmt 231.0.153.0`, plus aucune trace de `Failed to create HW CQ`,
et les liens ***REMOVED***92 et ***REMOVED***113 qui passent `pending → running` d'eux-mêmes — la réconciliation les
...
***REMOVED******REMOVED*** Si le flash ne change rien
