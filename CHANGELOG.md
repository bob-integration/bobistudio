...
flux vidéo ou de données auxiliaires perd un grain, parce que la bibliothèque réserve désormais la
plus ancienne case de l'anneau à celui qui écrit. Un lecteur qui lisait au ras de cette case
pouvait y voir une image à moitié écrite. L'audio n'est pas concerné : il fonctionnait déjà ainsi.

Nous ne l'affirmons pas, nous l'avons mesuré : sur un anneau de neuf grains, la fenêtre lisible
passe de neuf à huit, de façon stable. Et le contenu de chaque image a été vérifié par
l'empreinte du code source embarqué, pas par son numéro de version — un numéro se recopie, une
empreinte non.

**Un moteur 2110 qui revient sans ses abonnements se répare maintenant tout seul.** C'était le
défaut le plus sérieux de cette version, et il nous a coûté treize minutes d'antenne muette
pendant la mise à jour elle-même. Redémarrer un moteur lui fait perdre les fichiers qui décrivent
ce qu'il doit recevoir ; l'orchestrateur les lui repousse. Sauf qu'il ne réessayait que deux fois,
sur une minute, alors que le moteur met jusqu'à une minute rien qu'à démarrer sa pile réseau. Les
deux tentatives partaient dans le vide, une alerte s'affichait, et plus rien ne se passait — les
flux ne revenaient qu'après une intervention manuelle.

Deux corrections. La reprise tient désormais une échéance de cinq minutes et réessaie tant qu'elle
n'est pas atteinte, au lieu de compter ses essais. Et la surveillance, qui jusqu'ici se contentait
de signaler un moteur vide, tente la réparation — espacée d'une minute, limitée à cinq essais par
épisode, pour qu'une source amont éteinte ne fasse pas marteler le moteur indéfiniment. L'alerte
reste : un exploitant doit savoir qu'il s'est passé quelque chose, même réparé.

**Au passage**, une erreur de numéro dans le journal d'une image ne peut plus faire reconstruire
par-dessus une image de référence : les deux champs qui portent ce numéro doivent s'accorder,
sinon la construction est refusée. Elle l'était jusqu'ici choisie en silence.
...
qu'un contrôleur tiers découvre et inspecte. Bonne nouvelle : nous avions visé juste. Nos
ressources ont été **validées contre les schémas JSON de la spécification** plutôt que comparées
à l'œil, et elles passaient déjà, à trois détails près.

**Le premier n'était pas un détail.** La spécification exige IS-05 v1.2, nous servions la v1.1, et
ce n'est pas une formalité de numéro : dans la v1.1, le schéma des paramètres de transport est une
liste FERMÉE de quatre transports, où les paramètres MXL sont invalides. Un contrôleur qui valide
strictement rejetait donc nos ressources MXL — sans que rien, chez nous, ne le signale. Les deux
versions sont désormais servies : la v1.2 pour la conformité, la v1.1 conservée pour ne pas couper
un contrôleur ST 2110 qui y serait épinglé.

**Deux autres écarts ont été comblés.** L'endpoint `transporttype`, défini par IS-05, répondait 404
sur des ressources qui existent. Et 70 récepteurs de données auxiliaires n'annonçaient aucune
capacité BCP-004-01 : le code justifiait cela par l'absence de contrainte exprimable pour ce
format, ce qui était faux — un contrôleur les traitait donc comme n'ayant rien déclaré, et ne
pouvait rien confirmer avant de câbler.

**Le bus MXL a désormais son propre Device**, distinct de l'I/O ST 2110. Les deux surfaces
partageaient une seule entrée, nommée « 2110 I/O », qui portait en réalité 228 ressources MXL pour
18 ressources ST 2110 : un contrôleur n'avait aucun moyen de dire ce qu'il pouvait connecter, et
découvrait le refus après avoir essayé. Son nom se règle dans **Réglages → NMOS**.

**Ce qui reste ouvert est dit, pas caché.** Notre format vidéo interne planar n'est enregistré à
aucun registre — c'est un choix mesuré, un étage v210 en 10 bits dépasse le budget d'une image à
50 Hz — et le transport `urn:x-nmos:transport:mxl` lui-même n'y figure pas encore, trois semaines
après la publication de la spécification qui l'impose. Les deux points sont documentés dans la
référence `NMOS_MXL`, et remontés à l'AMWA.
