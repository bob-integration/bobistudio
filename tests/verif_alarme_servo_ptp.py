...
tout = True
***REMOVED*** 1) Démarrage : servo non convergé mais SOUS le seuil → silence
raz(); passe(dom(False))                       ***REMOVED*** seed muet
a = passe(dom(False))
tout &= cas("servo non convergé, sous le seuil", [], [x[0] for x in a])

***REMOVED*** 2) Au-delà du seuil → une alarme, UNE seule
time.sleep(2.2)
a = passe(dom(False))
...
***REMOVED*** 3) Reconvergence → retour à la normale
a = passe(dom(True))
tout &= cas("servo reconvergé", ['alert.ptp.servo_reconverge'], [x[0] for x in a])

***REMOVED*** 4) Nœud ptp4l (pas de PTP moteur) : jamais cette alarme
...
***REMOVED*** 5) Horloge ABSENTE : la disponibilité prime, pas d'alarme de qualité
...
***REMOVED*** 6) Servo convergé en permanence : silence total
