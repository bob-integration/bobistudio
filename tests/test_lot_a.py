...
Usage :
    /opt/bobistudio/venv/bin/python tests/test_lot_a.py          ***REMOVED*** sans pytest
    /opt/bobistudio/venv/bin/python -m pytest tests/test_lot_a.py -v   ***REMOVED*** si pytest présent
"""
import os
import re
import sqlite3
...
***REMOVED*** ─── DB temporaire AVANT toute utilisation (jamais la DB de prod) ─────────────
...
database.DB_PATH = _DB           ***REMOVED*** get_db() lit le global du module → surchargé ici

from app import mtl  ***REMOVED*** noqa: E402
from app import ptp  ***REMOVED*** noqa: E402
from app import settings as settings_mod  ***REMOVED*** noqa: E402

***REMOVED*** Settings mockés (render_switch_plan lit ptp_domain/ptp_hw_ts/ptp_client_only)
...
def _seed_db():
    ...
    ***REMOVED*** `settings` : le plan lit un réglage depuis que la vérification passe par `settings.get`.
    ***REMOVED*** Une table VIDE suffit — on veut les défauts du code, pas des valeurs de site — mais elle
    ***REMOVED*** doit EXISTER, sinon le test meurt sur « no such table » au lieu de vérifier quoi que ce soit.
    db.execute("CREATE TABLE IF NOT EXISTS settings (key TEXT PRIMARY KEY, value TEXT)")
    db.execute("DELETE FROM node_interfaces")
    ***REMOVED*** ⚠ `pair_role` EST OBLIGATOIRE pour qu'une paire 2022-7 existe. Le jeu d'essai ne posait
    ***REMOVED*** qu'un `pair_group`, ce qui suffisait AVANT que `vfio_bind_plan` ne soit corrigé : il
    ***REMOVED*** annonçait alors une « paire 2022-7 incohérente » à propos d'une paire INEXISTANTE (dl360-1
    ***REMOVED*** portait un groupe sans aucun rôle). Depuis, le garde exige groupe ET rôle des deux côtés,
    ***REMOVED*** et ce jeu d'essai ne décrivait plus aucune paire — le contrôle ne pouvait que rater.
    rows = [
        ***REMOVED*** (ifname, pci, role, pair_group, pair_role, ptp_enabled, pmd)
        ("ens1f0np0", "0000:12:00.0", "media2110", 1, "red",  0, None),   ***REMOVED*** candidat vfio
        ("ens1f1np1", "0000:12:00.1", "media2110", 1, "blue", 1, None),   ***REMOVED*** son pair, resté kernel
        ("eno1",      "0000:02:00.0", "management", None, None, 0, None), ***REMOVED*** mgmt (refus)
        ("ens2f0",    "0000:37:00.0", "media2110", None, None, 0, "dpdk"),***REMOVED*** déjà vfio (checklist)
    ]
...
def test_bind_plan_contenu_et_idempotence():
    ...
    ***REMOVED*** Vérification post-bind (échec explicite si le driver n'est pas vfio-pci)
    assert '"$NOW" = "vfio-pci"' in script and "@@ERR" in script
    ***REMOVED*** Persistance au boot : fichier + unité oneshot enabled
    assert mtl.VFIO_BINDS_PATH in script
    assert mtl.VFIO_BIND_UNIT_PATH in script
    ***REMOVED*** ★★★ GARDE DE NON-RÉGRESSION — NE PAS « RÉPARER » EN REMETTANT `enable`.
    ***REMOVED*** Ce contrôle exigeait `systemctl enable`. Il a été RETIRÉ du produit le 2026-08-28 : il
    ***REMOVED*** entraîne un `daemon-reload`, lequel réapplique la politique de périphériques des cgroups
    ***REMOVED*** et RÉVOQUE le GPU des conteneurs DÉJÀ EN MARCHE. Un moteur redémarré le 2026-08-19 a
    ***REMOVED*** ainsi tué le monitoring d'un utilisateur, relancé en boucle pendant NEUF JOURS.
    ***REMOVED*** Le lien d'activation est posé À LA MAIN, et seulement s'il manque.
    ***REMOVED*** Le contrôle est donc INVERSÉ : il échoue si `enable` revient.
    ...
    ***REMOVED*** Deux exécutions du même plan = même script (fonction pure)
    script2, _ = mtl.vfio_bind_plan(NODE, "0000:12:00.0")
    assert script == script2
    ***REMOVED*** Checks : éligibilité + persistance + warning pair 2022-7 laissé en kernel
...
def test_bind_plan_bdf_invalide_et_normalisation():
    ...
    ***REMOVED*** Forme courte + majuscules → normalisée (et matche la ligne node_interfaces)
...
***REMOVED*** ─── vfio_unbind_plan (symétrie / rollback) ───────────────────────────────────

def test_unbind_plan_symetrie():
    script, checks = mtl.vfio_unbind_plan(NODE, "0000:12:00.0")
    ***REMOVED*** Retour au driver kernel ice, avec le même schéma idempotent readlink→skip
    assert 'readlink "$DEV/driver"' in script
    assert '"$CUR" = "ice"' in script and "@@SKIP" in script
    ***REMOVED*** driver_override VIDÉ (echo > …), unbind du driver courant, re-probe, vérif ice
    assert re.search(r'^\s*echo > "\$DEV/driver_override"$', script, re.M), script
    assert '/unbind' in script
    assert "drivers_probe" in script
    assert '"$NOW" = "ice"' in script and "@@ERR" in script
    ***REMOVED*** Suppression de la persistance : retrait du BDF + purge unité si fichier vide
    assert f"sed -i" in script and mtl.VFIO_BINDS_PATH in script
    ***REMOVED*** Symétrique du bind : pas de `systemctl disable` (même raison), on retire le lien.
    ...
    ***REMOVED*** `rm -f` purge les trois chemins d'un coup (lien, unité, liste de BDF) : chercher
    ***REMOVED*** « rm -f <unité> » en un seul morceau ne correspond plus au texte produit.
    assert "rm -f " in script and mtl.VFIO_BIND_UNIT_PATH in script
    assert checks and "rollback" in checks[0]
    ***REMOVED*** Le rollback n'est JAMAIS refusé, même sur la NIC PTP ou management
    mtl.vfio_unbind_plan(NODE, "0000:12:00.1")
    mtl.vfio_unbind_plan(NODE, "0000:02:00.0")


***REMOVED*** ─── Runner injectable (verifier / apply à sec) ───────────────────────────────
...
def test_verifier_runner_injecte():
    ...
    assert not s["reboot_needed"]          ***REMOVED*** pending ⊆ live (tokens gérés)
...
def test_vfio_apply_a_sec_via_runner():
    ...
    ***REMOVED*** Échec remonté proprement
...
***REMOVED*** ─── render_switch_plan (préflight PTP Phase 1) ───────────────────────────────
...
def test_render_switch_plan():
    ...
    ***REMOVED*** Conf ptp4l : domaine + profil SMPTE rendus par _ptp4l_conf
    assert "domainNumber              127" in plan["ptp4l_conf"]
    assert "clientOnly                1" in plan["ptp4l_conf"]
    ***REMOVED*** Unités : la NOUVELLE interface est câblée dans ptp4l ET phc2sys (hw_ts)
