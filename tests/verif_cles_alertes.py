...
***REMOVED*** On repère d'abord les APPELS, puis on ramasse les clés DANS la fenêtre de l'appel. Deux motifs
***REMOVED*** naïfs ont été essayés et écartés :
***REMOVED***   · ancré sur le premier argument : un producteur sur deux choisit sa clé par une conditionnelle
***REMOVED***     (`"alert.x" if cause else "alert.y"`) et la seconde branche passait pour « sans producteur » ;
***REMOVED***   · tout littéral `alert.…`/`plugin.…` du fichier : ramasse `plugin.json` (un nom de fichier),
***REMOVED***     les exemples de docstrings, et les copies du dépôt dans les worktrees d'autres agents.
APPEL = re.compile(r"(?:db_add_alert|ajouter_alerte)\s*\(")
CLE = re.compile(r"""["'](alert\.[A-Za-z0-9_.]+)["']""")
FENETRE = 400          ***REMOVED*** de quoi couvrir un appel sur plusieurs lignes, pas le reste du fichier

***REMOVED*** Les deux contrôles n'ont PAS besoin de la même précision, et les confondre les rend faux tous
***REMOVED*** les deux :
***REMOVED***   · « clé manquante au catalogue » doit être STRICT — on ne veut pas alarmer sur un
***REMOVED***     `alert.foo` d'exemple dans une docstring. D'où la fenêtre autour d'un appel (MOTIF ci-dessus).
***REMOVED***   · « clé du catalogue sans producteur » doit être PERMISSIF — une clé peut parfaitement être
***REMOVED***     produite sans apparaître dans un appel : passée en `clef=` à un helper, rangée dans un tuple
***REMOVED***     `msg_on_i18n=("alert.rx.image_noire", {…})`, ou choisie par une variable calculée plus haut.
***REMOVED***     Exiger la fenêtre y produisait 31 fausses orphelines — un outil qui crie pour rien finit par
***REMOVED***     ne plus être lu.
***REMOVED*** Fragments `alert.…` trouvés N'IMPORTE OÙ dans le source, quote fermante NON exigée : une clé
***REMOVED*** sur six est COMPOSÉE (`f"alert.prep.controles_casses_{suffixe}"`, ou un suffixe `_sans_cible`
***REMOVED*** ajouté par un helper), et seul son PRÉFIXE apparaît littéralement. Une clé de catalogue est donc
***REMOVED*** tenue pour produite dès qu'un fragment du code en est un PRÉFIXE — sinon l'outil rapportait 50
***REMOVED*** fausses orphelines, et un outil qui crie pour rien finit par ne plus être lu.
LITTERAL = re.compile(r"(alert\.[A-Za-z0-9_.]*)")
***REMOVED*** `.claude` contient les worktrees d'autres agents : autant de COPIES du dépôt, qui feraient
***REMOVED*** compter chaque clé plusieurs fois et signaleraient des fichiers qui ne sont pas les nôtres.
...
def catalogue(code):
    ...
    try:
        fusion.update(json.load(io.open(chemin, encoding="utf-8")))   ***REMOVED*** le cœur prime
    except Exception as e:
...
def main():
    ...
    ***REMOVED*** Une clé de catalogue que plus aucun producteur n'émet n'est pas une erreur (elle a pu être
    ***REMOVED*** retirée d'un chemin de code), mais elle mérite d'être vue : c'est du catalogue mort.
    ...
    ***REMOVED*** `plugin.*` n'est volontairement PAS contrôlé ici : ces clés sont émises par du code qui
    ***REMOVED*** tourne DANS les conteneurs (avis de plugins), pas par ce dépôt — leur absence de producteur
    ***REMOVED*** visible est normale, pas un signalement.
    if orphelines:
