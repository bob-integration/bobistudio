...
***REMOVED***
***REMOVED*** Banc de `get.sh` : la ref d'un SOUS-MODULE n'est pas celle du produit.
***REMOVED***
***REMOVED*** ★ PANNE RÉELLE, 2026-09-02. `_recuperer` employait la MÊME ref pour le dépôt principal et pour
***REMOVED*** ses sous-modules. Tant qu'on installait « main », présente partout, ça passait. Le jour où l'on
***REMOVED*** a installé par ÉTIQUETTE, l'installation a échoué au premier sous-module : il n'existe aucun
***REMOVED*** « v0.9.3 » dans `bobistudio-service-nmos`, qui vit sur ses propres numéros.
***REMOVED***
***REMOVED*** Le message d'erreur, lui, était très bien écrit — il disait que le composant n'est pas
***REMOVED*** optionnel et suggérait de vérifier l'accès au dépôt. Il désignait donc la mauvaise cause, avec
***REMOVED*** assurance. Un bon message sur un mauvais diagnostic coûte plus cher que pas de message :
***REMOVED*** l'installateur a cherché du côté des droits, là où il n'y avait rien.
***REMOVED***
***REMOVED*** ⚠ AUCUN ACCÈS RÉSEAU. On vérifie la STRUCTURE du script et la logique de résolution, pas la
***REMOVED*** disponibilité de GitHub.
***REMOVED***
***REMOVED***   $ ./venv/bin/python tests/verif_get_sh_sous_modules.py
import io
import json
import os
import re
...
***REMOVED*** ★ LE CŒUR : l'APPEL à `_recuperer` dans la boucle des sous-modules doit porter une 4ᵉ ref.
***REMOVED***
***REMOVED*** ⚠ Ce contrôle a d'abord cherché la chaîne n'importe où dans le bloc de la boucle — et il était
***REMOVED*** MUET : `[ -n "$ref_sm" ]`, deux lignes plus haut, la contient aussi. Muté en retirant l'argument
***REMOVED*** de l'appel, le banc restait vert. On isole donc la LIGNE D'APPEL.
...
***REMOVED*** ═══ La logique de résolution, EXTRAITE DE get.sh et jouée hors ligne ═══════
***REMOVED***
***REMOVED*** ⚠ Une première version recopiait ce fragment dans le banc. Il était donc MUET : muter get.sh ne
***REMOVED*** changeait rien à ce qu'on exécutait. On teste ce que le script contient VRAIMENT.
...
***REMOVED*** ═══ AUCUN COMPOSANT N'EST BLOQUANT ════════════════════
***REMOVED***
***REMOVED*** ★ L'installation ABANDONNAIT si `services/nmos` était injoignable, en affirmant que
***REMOVED*** « l'orchestrateur ne démarrerait pas ». C'était vrai quand main.py l'importait sans garde ; ça
***REMOVED*** ne l'est plus. Et le remède suggéré — installer depuis le catalogue — exige que le produit
***REMOVED*** TOURNE : un cercle dont l'utilisateur ne peut pas sortir.
...
***REMOVED*** Le contrôle d'intégrité final ne doit plus exiger un composant facultatif : il annulerait
***REMOVED*** l'installation quelques lignes après qu'on ait décidé de continuer sans lui.
...
***REMOVED*** ═══ L'en-tête : version affichée, et cadre centré PAR CALCUL ═════════
***REMOVED***
***REMOVED*** C'est la première chose que voit quelqu'un qui installe le produit. Un cadre de travers y donne
***REMOVED*** le ton — et il l'a donné : le sous-titre était décalé de deux caractères, remplissage posé à
***REMOVED*** l'œil, signalé le 2026-09-02.
...
***REMOVED*** Lu dans le FICHIER, pas importé : ce banc ne doit dépendre d'aucun module du produit — il
***REMOVED*** vérifie un script autonome, téléchargé et exécuté avant que quoi que ce soit soit installé.
...
controle("★★★ le remplissage du cadre est CALCULÉ, pas compté à la main",
         "_cadre_ligne" in _src and "(l - ${***REMOVED***t}) / 2" in _src,
...
***REMOVED*** Le rendu, joué pour de vrai : c'est la seule preuve que le calcul centre.
...
***REMOVED*** ★ La langue choisie DOIT suivre : l'installeur la reposait trois secondes plus tard.
...
***REMOVED*** Toutes les clés employées doivent exister dans la table, et réciproquement.
...
***REMOVED*** ⚠ BORNÉ À L'ENTRÉE. Une première version cherchait `en=` n'importe où APRÈS la clé : elle
***REMOVED*** trouvait donc celle de la clé SUIVANTE, et restait verte quand une traduction disparaissait.
***REMOVED*** Chaque entrée d'un `case` bash finit par `;;` — c'est la borne.
...
***REMOVED*** ═══ LA PAUSE AVANT L'INSTALLEUR ═══════════════════════
