...
***REMOVED*** ── Auth agent par-conteneur (:8081) — rétro-compatible ───────────────────────
***REMOVED*** DEUX sources, dans cet ordre :
***REMOVED***   1. `containers.agent_token` — valeur ALÉATOIRE par conteneur, tirée et persistée par le driver
***REMOVED***      au moment du `docker run` qui l'injecte en MXL_AGENT_TOKEN (docker_compute / docker_driver).
***REMOVED***      C'est la source NORMALE depuis 2026-07.
***REMOVED***   2. REPLI : token DÉRIVÉ (HMAC-SHA256) du secret contrôleur (`flask_secret_key`) et du vmid —
***REMOVED***      le schéma historique. Il n'existe que pour les conteneurs créés AVANT la migration : leur
***REMOVED***      agent ne connaît de toute façon aucun token (l'injection n'était posée nulle part), donc
***REMOVED***      l'en-tête est ignoré. Ce repli est ce qui évite tout redéploiement forcé de la flotte ; la
***REMOVED***      bascule se fait conteneur par conteneur, au fil des redéploiements.
***REMOVED*** L'en-tête est envoyé best-effort : un agent SANS MXL_AGENT_TOKEN ignore l'en-tête inconnu ; un
***REMOVED*** agent AVEC le token l'exige (hmac.compare_digest).
***REMOVED***
***REMOVED*** POURQUOI on ne s'en tient pas au dérivé : `flask_secret_key` signe AUSSI les cookies de session
***REMOVED*** Flask. Tant qu'un conteneur en dépend, cette clé n'est PAS rotable — la faire tourner après un
***REMOVED*** incident rendrait toute la flotte impilotable. Traçabilité de la bascule :
***REMOVED*** `database.db_agent_token_etat()` (et le champ `agent_auth` de chaque conteneur).
...
***REMOVED*** Réglage HORS DEFAULTS (usage exceptionnel), volontairement non listé dans settings._BASE_DEFAULTS :
***REMOVED***   `agent_token_inject` = 0/false/off/no → on N'INJECTE PLUS MXL_AGENT_TOKEN au `docker run`.
***REMOVED*** ÉCHAPPATOIRE : activer une authentification jusque-là inopérante peut verrouiller l'orchestrateur
***REMOVED*** hors de ses propres conteneurs (un chemin d'appel qui oublierait l'en-tête). Ce réglage ramène au
***REMOVED*** comportement historique (agent OUVERT) SANS avoir à recréer quoi que ce soit à la main : le
***REMOVED*** poser, puis redéployer le conteneur concerné. Le token stocké est alors oublié
***REMOVED*** (`db_clear_agent_token`) pour que l'état de migration ne mente pas.
...
***REMOVED*** ── TLS du plan de contrôle vers l'agent conteneur :8081 (chantier feat/mtls) ──
***REMOVED*** Quand la CA interne est disponible, les conteneurs sont recréés HTTPS-only (cert conteneur signé
***REMOVED*** CA injecté au `docker run`, cf. docker_compute / agent.py). Le contrôleur parle alors à :8081 en
***REMOVED*** HTTPS : chaîne VÉRIFIÉE contre la CA, mais check_hostname DÉSACTIVÉ (connexion single-hop vers une
***REMOVED*** IP macvlan allouée au déploiement — on évite tout problème de timing d'IP SAN), + cert client du
***REMOVED*** contrôleur (mTLS). L'en-tête token X-MXL-Agent-Token reste envoyé EN PLUS (double facteur).
***REMOVED*** NB : ne concerne QUE :8081 (l'agent). Les :8080 (métriques) / :8082 (hot-wire) sont servis par le
***REMOVED*** script du plugin, restent en HTTP clair SANS AUTHENTIFICATION et ne passent PAS par ce helper —
***REMOVED*** c'est aujourd'hui le seul chemin d'écriture non authentifié du produit (:8082 pilote à chaud).
***REMOVED***
***REMOVED*** ⚠ CORRIGÉ le 2026-08-31 : ce commentaire affirmait que le moteur 2110 (`controller.py` de
***REMOVED*** l'image, --network host) « ne lit pas /etc/bobi-tls → hors périmètre, ses appels restent en
***REMOVED*** http ». C'EST FAUX, et ça l'était depuis un moment : `controller.py` a `_agent_tls_context()`,
***REMOVED*** et l'orchestrateur lui parle bien en `https://<nœud>:8081`. Le moteur est DANS le périmètre
***REMOVED*** mTLS comme les autres. Un commentaire périmé sur la sécurité est pire qu'aucun commentaire :
***REMOVED*** celui-ci aurait pu servir à justifier de ne pas protéger un chemin qui l'est déjà.
...
_agent_tls_enabled = None      ***REMOVED*** cache tri-état : None=non résolu, True/False
...
def _build_agent_ssl_context():
    ...
    ctx = ca.controller_client_context()   ***REMOVED*** cafile=CA + load_cert_chain(controller) [interface figée]
    ctx.check_hostname = False             ***REMOVED*** single-hop IP macvlan éphémère → pas de vérif de nom
    return ctx
...
def agent_session():
    ...
    if not agent_tls_on():
        return requests   ***REMOVED*** module : même API .get/.post ; comportement http historique
...
***REMOVED*** ── Filet : DÉSACCORD DE SCHÉMA contrôleur (https) ↔ agent conteneur (http) ────
***REMOVED*** POURQUOI (panne de prod 2026-07) : l'agent-nœud matérialise les PEM d'un conteneur sous
***REMOVED*** /run/bobi-tls/<nom>/ — or /run est un TMPFS. Au reboot du nœud : /run est vidé, Docker relève les
***REMOVED*** conteneurs `--restart unless-stopped` AVANT toute reprovision, la source du bind-mount n'existe
***REMOVED*** plus donc Docker la RECRÉE VIDE, et l'agent du conteneur (qui lit /etc/bobi-tls au démarrage) sert
***REMOVED*** en HTTP CLAIR. Le contrôleur, lui, a tranché HTTPS globalement (agent_tls_on, mémoïsé) → le
***REMOVED*** conteneur devient injoignable DÉFINITIVEMENT tout en continuant de tourner, et le seul symptôme
***REMOVED*** était un timeout indiscernable d'un conteneur mort.
***REMOVED***
***REMOVED*** Ce filet ne fait QUE du diagnostic : quand l'agent ne répond pas en HTTPS, on sonde UNE fois en
***REMOVED*** clair. S'il répond, le verdict est CERTAIN (le conteneur est vivant, c'est le schéma qui diverge)
***REMOVED*** et on le dit — alerte datée, niveau error. On ne bascule JAMAIS ce conteneur en HTTP : un repli
***REMOVED*** silencieux transformerait une panne visible en dégradation de sécurité permanente et muette.
***REMOVED*** La RÉPARATION est ailleurs (node_recovery : re-provision des certs au reboot).
_SCHEMA_ALERTE_PERIODE_S = 900          ***REMOVED*** anti-spam : au plus une alerte / conteneur / 15 min
_schema_alerte_ts = {}                  ***REMOVED*** vmid (ou ip) → time.monotonic() du dernier signalement
...
def diagnostiquer_schema_agent(ip, vmid=None, port=None, alerter=True):
    ...
    if not agent_tls_on():
        ***REMOVED*** Pas de CA → tout le monde est en http par construction : rien à désaccorder.
        return "ok" if _sonde_agent(f"http://{ip}:{_port}/status", vmid) else "injoignable"
    try:
        agent_session()      ***REMOVED*** matériel client du contrôleur lisible ? (sinon TOUT sera « muet »)
    except Exception as e:
        ***REMOVED*** Problème CÔTÉ CONTRÔLEUR (cert/clé illisibles) : ne pas l'imputer au conteneur.
...
def _attendre_agent_pret(ip, attempts=20, delay=0.5, vmid=None):
    ...
    import time
    _port = agent_port(vmid)   ***REMOVED*** :8081 par défaut, offsetté pour une sonde (CONTROLLER_PORT_BASE)
...
_MULTIVIEW_STRUCTURAL = frozenset({
    ...
    "orientation",   ***REMOVED*** change les dims/sens du flux émis (rotation 90°) → recrée le flux, pas de hot-apply
    ***REMOVED*** Lus à l'IMPORT du script (constantes de module) : un hot-apply ne les relit jamais, donc les
    ***REMOVED*** changer sans re-rendre le script ne fait RIEN — et `deployer_script` renvoie quand même True.
    ***REMOVED*** RESTAURÉ le 2026-08-11 : ces clés avaient été perdues (cf. note ci-dessous). Sans elles,
    ***REMOVED*** activer le mode tranche sur un mur hot-applique en silence : la base annonce `gpu_slice: True`,
    ***REMOVED*** le conteneur exécute un script qui n'en sait rien, et l'écart n'apparaît nulle part. Vérifié
    ***REMOVED*** hors ligne sur les six transitions de `gpu_slice` — les six rendaient « hot-apply ».
    "slice_mode", "slice_lines", "gpu_slice", "gpu_batch_bands",
    "force_cpu", "interlace_mode", "vars_check_s",
    ***REMOVED*** `cadence` manquait — MÊME PIÈGE que `gpu_slice`, trouvé le 2026-08-12. Elle est lue à
    ***REMOVED*** l'import (`CADENCE = str(CONFIG.get("cadence", "genlock"))`) et commande le mode FLOW,
    ***REMOVED*** c'est-à-dire le CIBLAGE D'INDEX de tout l'étage. La changer hot-appliquait : la base
    ***REMOVED*** annonçait `flow`, le conteneur exécutait un script qui n'en savait rien, et rien ne
    ***REMOVED*** signalait l'écart — `deployer_script` renvoyait True.
    "cadence",
})
        ...
        for entry in (params.get("flux_config") or []):
            ...
            if path:
                ...
                if fmt:
                    ...
                    ***REMOVED*** Format déclaré du producteur (chip « format source ») : toujours rafraîchi.
                    e["in_fps"], e["in_scan"] = fmt.get("fps") or "", fmt.get("scan") or "p"
            fc.append(e)
        ***REMOVED*** meter_blocks : blocs VU-mètres de MUR (indépendants des fenêtres, cf. render_meters) —
        ***REMOVED*** même remplacement atomique que flux_config, aucune résolution de dims/proxy nécessaire
        ***REMOVED*** (pas de vidéo, juste une source audio par bloc).
        r = _rq.post(f"http://{ip}:8082/reconfigure",
                     json={"flux_config": fc,
                           "meter_blocks": params.get("meter_blocks") or [],
                           ***REMOVED*** Blocs d'historique vidéo/audio (0.37.0) : mêmes conventions que
                           ***REMOVED*** meter_blocks — remplacement atomique, aucune résolution de dims.
                           "video_history_blocks": params.get("video_history_blocks") or [],
                           "audio_history_blocks": params.get("audio_history_blocks") or [],
                           ***REMOVED*** Polices de la bibliothèque référencées par la NOUVELLE config
                           ***REMOVED*** (injectées en base64 par fonts.resolve_params juste avant) : sans
                           ***REMOVED*** elles, un hot-apply qui introduit une police téléversée la ferait
                           ***REMOVED*** retomber sur DejaVu jusqu'au prochain redéploiement complet.
    ...
    for _c in db_get_containers():
        ...
        for _pr in _w.get("produces") or []:
            ...
            if not (_shm.startswith(src_name + "__p") and _f.get("width") and _f.get("height")):
                continue
            if _distant:
                ***REMOVED*** Pyramide d'un AUTRE nœud : demander la réplication (idempotent, dédupliqué)…
                try:
                    ...
                    log.warning("proxy %s : réplication vers le nœud %s non demandée (%s)",
                                _shm, node_id, _e)
                ***REMOVED*** … et n'injecter que si elle est EFFECTIVE (sinon on pointerait un flux absent).
...
_MAX_CUSTOM_PER_SRC = 6   ***REMOVED*** cap de tailles sur-mesure produites par source (anti-prolifération)

***REMOVED*** ─── Pyramide pilotée par la demande : état partagé (reconcile concurrent → sous _pyr_lock) ───
import time as _time
import threading as _threading
_pyr_lock = _threading.Lock()
_WIRE_GRACE_S = 10.0          ***REMOVED*** délai avant de décâbler une source qui n'est plus demandée (anti-flap)
_wire_absent_since = {}       ***REMOVED*** (vmid, slot) → t du 1er cycle « plus demandée »
_wire_cap_last = {}           ***REMOVED*** node_id → t du dernier avertissement « capacité insuffisante » (throttle 60s)
_LAT_HI_FRAC = 0.9            ***REMOVED*** latency_ms > 0.9*intervalle → saturation : interdire ajout + délester
_LAT_LO_FRAC = 0.8            ***REMOVED*** latency_ms < 0.8*intervalle → ré-autoriser les ajouts (hystérésis)
_pyr_capped = {}             ***REMOVED*** (vmid, src) → bool (état d'hystérésis du cap réactif)
_core_drift_seen = {}        ***REMOVED*** vmid → (have, want) déjà signalé (throttle alerte dérive cœurs)
_core_resize_done = {}       ***REMOVED*** vmid → want déjà déclenché en recréation auto (anti-boucle)
...
def _pyramide_runtime_inputs(c, st=None):
    ...
    if st is None:
        return None
    out = {}
    ***REMOVED*** ── FORME RÉELLE DU /state : {"inputs": {"0": "<shm>", "1": null, …}} ────────────────────
    ***REMOVED*** Dict IMBRIQUÉ, clés = numéro de slot en CHAÎNE, 0-based, valeur = nom de shm ou null.
    ***REMOVED*** Ce parcours cherchait des clés `input_N` de PREMIER NIVEAU, que la pyramide n'émet pas :
    ***REMOVED*** il rendait donc TOUJOURS {} → l'appelant concluait « câblage runtime perdu », re-poussait
    ***REMOVED*** tout le câblage et écrivait une alerte, À CHAQUE TOUR. Mesuré le 2026-08-15 : 879 alertes,
    ***REMOVED*** dont 637 sur les seules 24 h — pour une pyramide dont le câblage était parfaitement en place.
    ***REMOVED*** Un lecteur qui ne sait pas lire son témoin ne rapporte pas « je ne sais pas » : il rapporte
    ***REMOVED*** « tout est cassé », et déclenche une réparation permanente.
    src = st.get("inputs")
    if isinstance(src, dict):
        for k, v in src.items():
            if not v:
                continue                       ***REMOVED*** slot libre : absent du résultat, comme avant
            k = str(k)
            if k.isdigit():
                out[int(k)] = v.split("/")[-1] if isinstance(v, str) else v
        return out
    ***REMOVED*** Repli : forme À PLAT `input_N` au premier niveau (jamais observée sur la pyramide, gardée
    ***REMOVED*** au cas où un autre producteur adopterait ce contrat).
    for k, v in st.items():
...
def _reconcile_pyramide_wiring(node_id, conts, wanted_sources):
    ...
    now = _time.time()
    ***REMOVED*** sources déjà câblées quelque part sur le nœud
    wired = set()
    for (_c, _dc, p) in pyrs:
        for i in range(_n(p)):
            ...
            if s:
                wired.add(s)
    ***REMOVED*** 0. Backfill du format des slots DÉJÀ câblés sans `input_{i}_fmt` persisté (résolu via DB,
    ***REMOVED***    idempotent) → derive_wiring peut lister les proxies sur-mesure (console/topologie).
    for (c, dc, p) in pyrs:
        changed = False
        for i in range(_n(p)):
            s = p.get(cle_input(i))
    ...
    ***REMOVED*** 0b. RESYNC RUNTIME. Le câblage d'une pyramide est HOT (`mode: hot-wire`) : il vit dans le
    ***REMOVED***     process, pas sur disque. Tout redémarrage du script — SIGBUS d'un producteur, restart de
    ***REMOVED***     l'agent, reboot du nœud, simple stop/start — le PERD, pendant que la DB continue
    ***REMOVED***     d'affirmer que les slots sont câblés. Le reconcile, qui ne lisait que la DB, se croyait
    ***REMOVED***     alors idempotent (`changed: []`) et laissait la pyramide à **0 fps DÉFINITIVEMENT** ; les
    ***REMOVED***     consommateurs, eux, cessaient en silence de trouver des proxies et repassaient au plein
    ***REMOVED***     format. Aucun signal nulle part — l'anti-patron de l'échec silencieux, constaté au banc le
    ***REMOVED***     2026-07-29 après un simple stop/start du script. Même remède que pour le moteur 2110 après
    ***REMOVED***     recréation (cf. resync_moteur) : c'est le RUNTIME qui fait foi pour savoir ce qui est
    ***REMOVED***     appliqué, la DB ne dit que ce qui est VOULU.
    for (c, dc, p) in pyrs:
        live = _pyramide_runtime_inputs(c)
        if live is None:
            continue                      ***REMOVED*** injoignable : on ne conclut rien (jamais sur du vide)
    ...
    ***REMOVED*** 1. Libérer les slots dont la source n'est plus demandée (grâce anti-flap)
    for (c, dc, p) in pyrs:
        vmid = c.get("vmid")
        for i in range(_n(p)):
            s = p.get(cle_input(i))
            ...
            if s and s not in wanted_sources and grace_ok:
                if _set_pyramide_input(c, i, None, None):
                    ...
                    with _pyr_lock:
                        _wire_absent_since.pop((vmid, i), None)
    ***REMOVED*** 2. Câbler les sources demandées non encore câblées
    unplaced = []
...
def _check_pyramide_core_drift(c, params, read_count):
    ...
    if have == 0 or have == want:
        return
    ***REMOVED*** Tous les proxies de cette pyramide orphelins ? (aucun consommateur)
    consumers = 0
    try:
        ...
        consumers = 1   ***REMOVED*** en cas de doute, ne pas recréer
    if consumers == 0:
        with _pyr_lock:
        ...
        if not already:
            ...
            ***REMOVED*** Redéploiement complet (deploy_compute fait `docker rm -f` + run → release+realloc du cpuset).
            ***REMOVED*** En thread pour ne pas bloquer le reconcile. Anti-boucle : _core_resize_done garde le `want`.
            try:
        ...
        return
    ***REMOVED*** Consommateurs présents → pas de coupure : alerte (throttlée sur le couple have/want)
    with _pyr_lock:
...
def _apply_reactive_cap(vmid, slot, src, kept, old_extra):
    ...
    if not live or live.get("latency_ms") is None or not (live.get("fps") or 0):
        return kept   ***REMOVED*** conteneur pas up / source figée → ne pas caper
    ...
    kept = [b for b in kept if (b[0], b[1]) in old_set]   ***REMOVED*** interdit tout AJOUT sous saturation
    if kept:
        kept = kept[:-1]                                  ***REMOVED*** déleste la taille la moins demandée
    if not was:
...
def pousser_telemetrie():
    ...
    def _bloc(sn, nom=None, node_id=None):
        ...
        if nom:
            out["nom"] = nom
        ***REMOVED*** RDMA : REMPLISSAGE des liens de réplication. Le trafic est la somme des ports actifs, le
        ***REMOVED*** dénominateur leur débit NOMINAL cumulé — c'est la question « reste-t-il de la place ? »,
        ***REMOVED*** celle qu'on se pose avant d'y faire passer un flux de plus. Un nœud sans RDMA ne publie
        ***REMOVED*** rien plutôt que zéro : « 0 % » laisserait croire à un lien vide alors qu'il n'y en a pas.
        try:
    ...
    for c in db_get_containers():
        ...
        ***REMOVED*** Le nœud de CE mur, le contrôleur, ET tous les nœuds nommés — pour qu'un mur de
        ***REMOVED*** supervision puisse afficher n'importe lequel via `%cpu_noeud:dl360-1%`. Le parc tient en
        ***REMOVED*** quelques nœuds : envoyer le tout coûte moins qu'un protocole d'abonnement.
        _tous = {}
...
def reconcile_pyramide_sizes(node_id):
    ...
    try:
        ...
        return {"changed": []}
    ***REMOVED*** DEMANDE INTER-NŒUD. Une pyramide sert les sources produites SUR SON NŒUD, à qui que ce soit —
    ***REMOVED*** y compris un consommateur d'un autre nœud. C'est tout l'intérêt de réduire À LA SOURCE :
    ***REMOVED*** ne faire traverser que des proxies (~4,8× moins de données qu'un 1080p répliqué pour être
    ***REMOVED*** réduit à l'arrivée). Sans cet élargissement, une pyramide placée près des sources n'apprend
    ***REMOVED*** jamais ce qu'on lui demande et reste à vide — constaté le 2026-08-07 sur dl360-1.
    ***REMOVED*** On ne retient d'un consommateur DISTANT que les sources produites ICI : chaque pyramide
    ***REMOVED*** reste responsable de son propre voisinage.
    _prod_ici = set()
    for _c in conts:
    ...
    ***REMOVED*** 1. Besoins agrégés par source : src -> [(w,h,count)…]
    agg = {}
    for c in conts + _distants:
        ...
        for src, items in needs.items():
            if _loin and src not in _prod_ici:
                continue      ***REMOVED*** source d'un autre voisinage : ce n'est pas à cette pyramide-ci
            for it in (items or []):
                try:
    ...
    ***REMOVED*** 1b. AUTO-CÂBLAGE : les sources réclamées (clés de proxy_needs) sont assignées aux slots de
    ***REMOVED*** pyramide libres du nœud (hot), les inutiles libérées. Mute deploy_config (DB + conts) → la
    ***REMOVED*** boucle extra_sizes ci-dessous re-parse et voit le câblage à jour.
    ...
    ***REMOVED*** read_count : nb de consommateurs par shm (pour la détection « pyramide orpheline » de la dérive cœurs).
    read_count = {}
    try:
        from . import metrics as _m
        ...
        read_count = {}
    ***REMOVED*** 2. Mise à jour des pyramides du nœud
    changed = []
    for c in conts:
        try:
            dc = _json.loads(c.get("deploy_config") or "{}") or {}
        ...
        for i in range(n):
            ...
            buckets.sort(key=lambda b: -b[2])   ***REMOVED*** count décroissant = priorité
            ***REMOVED*** SEUIL SOUPLE : on sert TOUTES les tailles demandées jusqu'au cap (les tailles
            ***REMOVED*** meter-réduites demandées par UNE seule fenêtre — non-diviseur-entier de la source —
            ***REMOVED*** tombaient sinon en `gather`, le cas exact que la pyramide doit éliminer). Le cap borne
            ***REMOVED*** la prolifération ; `custom_size_threshold` ne départage QUE le surplus quand il y a
            ***REMOVED*** PLUS de tailles distinctes que le cap (sinon il n'écarte plus rien).
            if len(buckets) > _MAX_CUSTOM_PER_SRC:
                buckets = [b for b in buckets if b[2] >= thr]
            kept = buckets[:_MAX_CUSTOM_PER_SRC]
            kept = _apply_reactive_cap(vmid, i, src, kept, old_extra)   ***REMOVED*** CAP RÉACTIF (latence)
            if kept:
                new_extra[src] = [[w, h] for (w, h, _cnt) in kept]
        ***REMOVED*** Dérive cœurs (alerte, ou recréation auto si aucun consommateur) — toujours évaluée.
        try:
            _check_pyramide_core_drift(c, p, read_count)
        except Exception as _e:
            log.warning("dérive cœurs pyramide %s: %s", vmid, _e)
        ***REMOVED*** Le garde-fou d'idempotence compare le VOULU à la DB — mais la DB ne dit que ce qu'on a
        ***REMOVED*** demandé, jamais ce que le process produit. Les tailles sur-mesure sont du RUNTIME au même
        ***REMOVED*** titre que les slots d'entrée : un redémarrage du script les efface, et avec
        ***REMOVED*** `base_octaves: none` elles sont TOUTE la production → la pyramide ne publie plus RIEN
        ***REMOVED*** pendant que la DB affirme le contraire. Constaté au banc le 2026-07-29 : 8 tailles en base,
        ***REMOVED*** `/state.proxies == []`. On ne saute donc le hot-apply que si la DB est à jour ET que le
        ***REMOVED*** runtime produit effectivement les proxies attendus.
        ...
        if not ip:
            continue
        try:
            ***REMOVED*** HOT-APPLY : POST :8082/extra_sizes → le worker crée/retire les proxies concernés SANS
            ***REMOVED*** redémarrer (aucune coupure des proxies inchangés). Puis persiste dans deploy_config
            ***REMOVED*** (sans restart) pour survivre à un futur redéploiement.
...
def refresh_consumers_of_source(producer_vmid):
    ...
    try:
        ...
        return
    ***REMOVED*** shm(s) produits par la source
    prod_shms = set()
    try:
    ...
    for c in db_get_containers():
        ...
        if ctype == "multiview":
            ...
            ***REMOVED*** Sortie TX consommant un shm de la source ? (state_field tx{i}_shm / tx_audio{i}_shm /
            ***REMOVED*** tx_anc{i}_shm). Si oui → re-pousse les slots : le contrôleur re-résout le format du flux
            ***REMOVED*** courant et se ré-abonne (corrige le « garbage » après recréation du flux producteur).
            ...
            ***REMOVED*** Streamer lisant ce flux (shm_name vidéo ou audio_shm) → re-déploiement = reconnexion.
...
def deployer_script(vmid, type_script, params, script_path="/opt/script/main.py", version=None,
    ...
    with verrou_vmid(vmid, op="deploy"):
        db_set_desired_state(vmid, "running")   ***REMOVED*** un déploiement = intention « ça doit tourner »
        ***REMOVED*** Un déploiement ne crée PLUS d'emplacement. Un emplacement est une position de
        ***REMOVED*** production (« MULTIVIEW RÉGIE 1 »), donc une décision humaine ; le dériver
        ***REMOVED*** automatiquement d'un conteneur confondait le 3e barreau d'identité avec le 2e, et
        ***REMOVED*** produisait un libellé — le hostname — qui est justement ce qu'un emplacement ne doit
        ***REMOVED*** pas être. Mesuré le 2026-08-30 : 282 emplacements semés, 8 servis, et 2 des survivants
        ***REMOVED*** jamais renommés. Ils se créent maintenant depuis Réglages → Ember+ → Emplacements.
...
def _deployer_script_locked(vmid, type_script, params, script_path="/opt/script/main.py",
    ...
    ***REMOVED*** `h` = hostname : sans lui, le suivi de création par lot ne peut pas rattacher cette alerte
    ***REMOVED*** à sa ligne (il apparie par `msg_params.h`, cf. `_lastAlertFor` dans static/scripts.js) et
    ***REMOVED*** l'étape « Configuration » resterait inatteignable. La lecture est déjà faite quelques lignes
    ***REMOVED*** plus bas ; on l'avance, plutôt que d'ajouter un aller-retour en base.
    ...
    ***REMOVED*** Backend Docker : deux chemins. MTL = controller bâti dans l'image, config par NMOS, pas
    ***REMOVED*** d'agent :8081 ni de rendu script.py → chemin dédié. Compute générique = conteneur agent
    ***REMOVED*** macvlan : on le garantit up ici puis on POURSUIT sur le chemin agent standard (comme un LXC).
    _cd = _c0
    if _cd:
        ...
        if not docker_compute.deploy_compute(vmid, params, deploy_type=type_script):
            return False
        ***REMOVED*** … pas de return : on tombe dans le rendu + POST :8081/deploy ci-dessous.
    ...
    ***REMOVED*** Notify NMOS dès le début du deploy pour que le registre reflète rapidement
    ***REMOVED*** tout changement de type ou de topologie. Le rebuild_model() post-deploy
    ***REMOVED*** (après db_update_deploy_config) reste le notify authoritative.
    ...
    ***REMOVED*** Producteurs nommés par hostname (mixer, correcteur) : on injecte le hostname
    ***REMOVED*** réel du container dans params pour que scripts.py et la dénormalisation
    ***REMOVED*** shm_out plus bas tombent sur le même nom que la topologie home (routes.py),
    ***REMOVED*** qui dérive le shm producteur depuis c.hostname. Sans ça, le câble pointe
    ***REMOVED*** sur melangeur_pgm mais le script écrit sur mxl_pgm → écran noir.
    if plugins.is_plugin(type_script):
        c = db_get_container(vmid) or {}
        params = dict(params)
        params.setdefault("hostname", c.get("hostname") or f"mxl{vmid}")

    ***REMOVED*** (Le pool SR-IOV nic_pool et l'allocation VF par-conteneur ont été retirés : le moteur
    ***REMOVED*** 2110_io tourne sur la PF en AF-XDP, --network host, sans VF.)

    ***REMOVED*** Hook before_deploy : normalisation déclarée par le plugin (hooks.py).
    ***REMOVED*** Reçoit params + contexte incluant tous les settings orchestrateur (lecture seule).
    ***REMOVED*** Un hook qui lève une exception est ignoré ; le deploy continue.
    ...
    ***REMOVED*** Plugins qui déclarent `brand` : on injecte l'identité (perso) dans leurs params.
    if plugins.is_plugin(type_script) and plugins.wants_brand(type_script):
        params = _resolve_brand_settings(params)

    ***REMOVED*** (Bibliothèque de polices : voir `_gras()` — les params PERSISTÉS restent MAIGRES, seuls
    ***REMOVED*** les params ENVOYÉS au conteneur portent le base64.)

    ***REMOVED*** Injecte les ring sizes depuis les settings (lus ici pour être dans le CONFIG du script).
    if plugins.is_plugin(type_script):
        from . import settings as _st_ring
        params = dict(params)
        ***REMOVED*** Ring vidéo : suit le réglage (borné [2:8] par le formulaire, MTL st20 ≤8). Le réglage
        ***REMOVED*** est l'unique source de vérité ; pas de garde-fou ici (fallback aligné sur le défaut 8).
        ...
        ***REMOVED*** Profondeur de bits du pipeline shm : arbitrage orchestrateur. Le script ne connaît
        ***REMOVED*** pas le mode (force8/follow), il ne reçoit qu'un `bit_depth` ∈ {8,10,12} dans CONFIG.
        _apply_pipeline_bit_depth(params, _st_ring.get("mxl_pipeline_bit_depth") or "force8")

    ***REMOVED*** Multiview : résoudre in_w/in_h (+ in_fps/in_scan, format déclaré du producteur,
    ***REMOVED*** pour le chip « format source ») de chaque fenêtre depuis la DB (même logique que
    ***REMOVED*** _apply_wire). Corrige les valeurs laissées au défaut 640×360 quand le déploiement
    ***REMOVED*** vient de l'éditeur plutôt que de l'outil câble.
    if type_script == "multiview":
        ...
        for _i, _entry in enumerate(_fc):
            ...
            if _fmt:
                ...
                _new.update(_vals)
            ***REMOVED*** Pyramide (Phase 2, OPPORTUNISTE) : si une pyramide CO-LOCALISÉE (même nœud =
            ***REMOVED*** même /dev/shm) descale cette source, injecter ses proxies {path,w,h}. Le script
            ***REMOVED*** multiview choisit à l'exécution le proxy le mieux dimensionné par tuile (sinon
            ***REMOVED*** plein). Absent → on retire un éventuel `proxies` périmé (retour au classique).
    ...
    ***REMOVED*** Multiview : hot-apply si seuls des params style/géo/source ont changé.
    ***REMOVED*** Compare avec les params actuellement en DB ; si aucun param structurel n'a
    ***REMOVED*** bougé et que le script répond, on évite le redéploiement (pas de coupure sortie).
    ***REMOVED*** GARDE « script réellement en marche » : le rootfs des conteneurs est ÉPHÉMÈRE — après un
    ***REMOVED*** docker restart, /opt/script est vide (agent /status → running:false, path:null). Sans cette
    ***REMOVED*** garde, la branche MUR SHARDÉ ci-dessous retournait True sans jamais redéployer de script
    ***REMOVED*** (le tissu ne fait que du hot-apply :8082) → mur définitivement stoppé (vu sur le 163).
    if (type_script == "multiview" and ip and script_content is None
        ...
        if _old_cd:
            ...
            ***REMOVED*** Hot-apply seulement si le script déployé est À LA version du manifeste :
            ***REMOVED*** un vieux script ignorerait les nouveaux champs (ex. hidden → PiP fantômes).
            _cur_ver = (plugins.get(type_script) or {}).get("version")
            ***REMOVED*** ÉTAT OBSERVÉ d'abord : ce que le conteneur EXÉCUTE prime sur ce que la base
            ***REMOVED*** prétend. Un script antérieur à 0.68.0 ne publie pas sa version → None → on
            ***REMOVED*** retombe sur l'estampille (comportement historique, aucune régression), et le
            ***REMOVED*** premier redéploiement rend le mur vérifiable pour toujours.
            ...
            if (_ver_effective == _cur_ver
                ...
                if _dfg(f"asm:{vmid}"):
                    ***REMOVED*** Mur SHARDÉ : le conteneur logique est l'ASSEMBLEUR (copie pure des shards
                    ***REMOVED*** pré-rendus). Ne JAMAIS lui hot-appliquer les params logiques : le chrome se
                    ***REMOVED*** dessinerait autour de chaque BLOC de shard, et le flux_config logique
                    ***REMOVED*** écraserait le tuilage. Canal correct = persister puis RE-PLANIFIER le tissu
                    ***REMOVED*** (les signatures de cellule incluent le style → shards re-matérialisés,
                    ***REMOVED*** assembleur reconfiguré). Pousser /style aux shards directement est aussi
                    ***REMOVED*** proscrit : un shard PARTAGÉ entre deux murs serait corrompu pour l'autre.
                    ...
                    db_add_alert("alert.deploy.multiview_shard_replanifie", "info", vmid=vmid, kind="deploy",
                                 params={"vmid": vmid})
                    return True
                if _multiview_hot_apply(ip, _gras(params)):   ***REMOVED*** gras à l'ENVOI, maigre en base
                    ***REMOVED*** Le script n'a PAS changé (hot-apply) → il tourne toujours en _cur_ver.
                    ***REMOVED*** On reporte plugin_version (l'éditeur ne l'envoie pas) sinon le PROCHAIN
                    ***REMOVED*** hot-apply échouerait la garde de version et forcerait un redéploiement.
                    params = {**params, "plugin_version": _cur_ver}
                    db_update_deploy_config(vmid, type_script, params)
    ...
    if script_content is None:
        ***REMOVED*** Rendu du script = params GRAS (polices en base64) ; `params` reste MAIGRE pour la
        ***REMOVED*** persistance et les comparaisons (cf. _gras).
    ...
    ***REMOVED*** Attendre que le contrôleur :8081 écoute (LXC qui démarre / conteneur Docker dont le HTTPServer
    ***REMOVED*** n'est pas encore up) — évite l'alerte « Connection refused » lors d'une (re)création de container.
    if not _attendre_agent_pret(ip, vmid=vmid):
        ...
        ***REMOVED*** Filet mTLS : muet en HTTPS ≠ conteneur mort. Une sonde en clair tranche (et alerte si
        ***REMOVED*** c'est un désaccord de schéma) — sans quoi on repart sur un POST /deploy qui échouera
        ***REMOVED*** avec un message de timeout n'expliquant rien.
        diagnostiquer_schema_agent(ip, vmid=vmid)

    try:
        ***REMOVED*** bobimxl.py POUSSÉ AVEC LE SCRIPT (modèle « script poussé » étendu au binding) : déposé
        ***REMOVED*** à côté de main.py, il SHADOWE le module baké dans l'image runtime (sys.path[0] =
        ***REMOVED*** /opt/script) → les évolutions PUR-PYTHON du binding (codec ANC RFC 8331, data flows…)
        ***REMOVED*** atteignent la flotte au redéploiement, SANS rebuild d'image. La compatibilité avec la
        ***REMOVED*** libmxl.so de l'image reste requise (le binding ctypes vérifie ses symboles) — validé
        ***REMOVED*** sur les images actuelles (round-trip ANC dans bobi-compute). Best-effort : un échec
        ***REMOVED*** n'empêche pas le déploiement (le script retombe sur le bobimxl de l'image).
        try:
        ...
        if r.status_code == 200:
            ...
            ***REMOVED*** source/shm : hook source_shm du plugin, ou fallback wiring déclaratif
            _ss_hook = plugins.get_hook(type_script, "source_shm")
            if _ss_hook:
                try:
                    ***REMOVED*** Import local : `_st` n'est lié plus haut que si le plugin a un hook
                    ***REMOVED*** before_deploy — un plugin qui n'a QUE source_shm levait UnboundLocalError.
                    from . import settings as _st
            ...
            ***REMOVED*** Versioning : enregistre la version du plugin dans deploy_config (JSON, pas
            ***REMOVED*** de migration SQL). Permet "version par container déployé" + détection drift.
            if plugins.is_plugin(type_script):
                ***REMOVED*** Version réellement rendue (la demandée si archivée connue, sinon courante).
                params["plugin_version"] = plugins.resolved_version(type_script, version)

            db_update_source(vmid, source, shm)
            db_update_deploy_config(vmid, type_script, params)
            ***REMOVED*** Re-notifier NMOS APRÈS l'écriture du deploy_config : rebuild_model
            ***REMOVED*** détecte les senders/receivers depuis deploy_config.type, donc le
            ***REMOVED*** notify précoce (haut de fonction) tombe sur l'ancienne config et
            ***REMOVED*** n'expose pas le flux fraîchement déployé.
            try:
            ...
            ***REMOVED*** « Plan 2 » : pousser au CONTENEUR la description de ses propres ressources NMOS, pour
            ***REMOVED*** qu'il serve son Node API sur son :8081 (cf. services/nmos/conteneur_node.py). Ici et
            ***REMOVED*** pas plus haut : `document()` lit `deploy_config`, qui vient seulement d'être écrit —
            ***REMOVED*** pousser avant enverrait la description de la config PRÉCÉDENTE.
            ***REMOVED*** Best-effort et fermé par défaut : un agent d'ancienne image ne connaît pas /nmos, et
            ***REMOVED*** un déploiement ne doit jamais échouer pour cette raison.
            try:
            ...
            ***REMOVED*** Pyramide : un multiview (re)déployé peut changer ses besoins de tailles → reconcile
            ***REMOVED*** différé (le temps qu'il publie proxy_needs sur :8080), hot-apply, no-op si inchangé.
            if type_script == "multiview":
                _schedule_pyramide_reconcile(vmid)
                ***REMOVED*** Tissu : sharder un mur lourd SANS attendre le tick de 30 s (sinon il sature le bus
                ***REMOVED*** mémoire en monolithe et pénalise les sorties live). No-op si fabric_auto off / mur
                ***REMOVED*** interne / pas saturé. Sérialisé par le verrou par-nœud (anti-race avec le tick).
                _schedule_fabric_reconcile(vmid)

            ***REMOVED*** Changement de format ORCHESTRÉ : si cette (re)déploiement modifie le format d'une
            ***REMOVED*** source, rafraîchir à chaud les multiviews qui la consomment (in_w/in_h + /reconfigure).
            ***REMOVED*** Différé (shm recréé à sa nouvelle taille) ; no-op si pas de consommateur / dims inchangées.
            _schedule_consumer_refresh(vmid)

            ***REMOVED*** Relancer le script pour qu'il prenne le nouveau code
            try:
                agent_session().post(agent_url(ip, "/stop"), timeout=5, headers=agent_headers(vmid))
            except Exception:
                pass
            ***REMOVED*** ★ SAUF SI L'EXPLOITATION L'A DÉSACTIVÉ. Le script est déployé (le code est à jour et
            ***REMOVED*** prêt), mais il n'est PAS démarré : `containers.script_enabled` porte l'intention,
            ***REMOVED*** posée par le `NcWorker.enabled` du modèle MS-05-02. Sans ce garde, un contrôleur NMOS
            ***REMOVED*** tiers verrait son arrêt accepté puis défait au premier redéploiement — le pilotage
            ***REMOVED*** aurait l'air de marcher et ne marcherait pas.
            from .database import db_script_enabled
            _voulu = db_script_enabled(vmid)
            if not _voulu:
                ***REMOVED*** Déployé et prêt, volontairement PAS démarré. On le DIT : un script à l'arrêt sans
                ***REMOVED*** trace se lit comme une panne, et quelqu'un ira le relancer à la main.
...
def rewire_on_restart(vmid):
    ...
    try:
        ...
        if not t or not plugins.is_plugin(t):
            return
        ***REMOVED*** Moteur MTL (2110_io) : son état vit sur :8081 (slots TX + abonnements RX IS-05), PAS sur
        ***REMOVED*** le hot-wire :8082 générique. On rejoue le MÊME resync que deploy_docker, sinon un restart
        ***REMOVED*** du conteneur laisse senders=0 → SDP TX en 404 (sorties NMOS non abonnables) et les RX ne
        ***REMOVED*** sont rétablis que par un contrôleur NMOS externe. push_tx_slots/repush gèrent le retry
        ***REMOVED*** le temps que :8081 réponde (conteneur qui vient de démarrer).
        from . import docker_compute
        if docker_compute.is_mtl_type(t):
            ***REMOVED*** MÊME séquence VÉRIFIÉE que le (re)déploiement (docker_driver.resync_moteur) :
            ***REMOVED*** readiness bornée → slots TX → abonnements RX → vérification + ALERTE si le moteur
            ***REMOVED*** reste vide. L'ancien code enchaînait les deux pushes sans readiness commune ni
            ***REMOVED*** vérification → un moteur relancé par la surveillance revenait muet EN SILENCE.
            from . import docker_driver
        ...
        for spec in (w.get("consumes") or []):
            ...
            ***REMOVED*** state_field peut contenir {i1} (ex. "input_{i1}") — résout sur les slots 0..n, la
            ***REMOVED*** CLÉ étant 1-based (cf. app/numerotation.py). `i` reste l'indice de slot ; seul le
            ***REMOVED*** numéro écrit dans la clé est décalé — d'où `numero(i)` et non `i`.
            if "{i1}" in sf:
                i = 0
                while True:
...
***REMOVED*** Verrou par-nœud des reconciles de tissu : le tick périodique (main.py) ET le hook au (re)deploy
***REMOVED*** (_schedule_fabric_reconcile) peuvent viser le même nœud en même temps → sans sérialisation ils
***REMOVED*** racent sur l'API (double matérialisation de shards). Non-bloquant : un reconcile en cours suffit.
_fabric_node_locks = {}
...
***REMOVED*** ─── ANNULATION D'UNE RÉPLANIFICATION DEVENUE OBSOLÈTE ──────────────────────────────────────
***REMOVED*** Composer un layout, c'est déplacer trente choses en deux minutes. Chaque édition lançait une
***REMOVED*** réplanification qui allait AU BOUT — planifier, créer un conteneur, attendre qu'il produise,
***REMOVED*** basculer, détruire le précédent — pour être remplacée quatre secondes plus tard par la
***REMOVED*** suivante. Mesuré le 2026-08-07 : six conteneurs créés en trente secondes, chacun obsolète
***REMOVED*** avant même d'avoir servi. Aucun n'était inutile quand il a été décidé ; ils l'étaient devenus
***REMOVED*** PENDANT leur fabrication.
***REMOVED*** Chaque édition incrémente donc la génération du nœud, et une réplanification renonce dès
***REMOVED*** qu'elle se sait dépassée — aux points où renoncer ne casse rien. On ne fait jamais patienter
***REMOVED*** l'opérateur : c'est le travail périmé qu'on supprime, pas ses gestes.
_fabric_gen = {}
...
def _fabric_refresh_wall(vmid):
    ...
    if _nid is None:
        return
    _gen = _fabric_bump(_nid)     ***REMOVED*** AVANT le fil : les réplanifications en vol sont déjà périmées

    def _run():
        try:
            with _fabric_lock(_nid):
                ***REMOVED*** Une édition PLUS RÉCENTE est arrivée pendant qu'on attendait le verrou : elle
                ***REMOVED*** planifiera à partir d'un état plus juste que le nôtre. On se retire.
    ...
    def _run():
        try:
            ...
            if str(c.get("hostname") or "").startswith("bobi-fab-"):
                return   ***REMOVED*** nœud interne du tissu (shard/assembleur) → pas de re-déclenchement
...
***REMOVED*** Un mur déployé depuis moins que ça n'est pas jugé par le tissu : sa latence est celle de la
***REMOVED*** rafale de re-bake, pas de son régime. Deux minutes couvrent le re-bake d'habillage plein cadre
***REMOVED*** et la recomposition des frises, mesurés sur un mur de 4 cellules richement habillé.
_DELAI_JUGEMENT_S = 120.0

***REMOVED*** ─── Confirmer avant de sharder, et savoir en sortir ─────────────────────────────────────────
***REMOVED*** ★ POURQUOI CES DEUX CONSTANTES EXISTENT (2026-08-08, mur 906 sur dell-1).
***REMOVED***
***REMOVED*** Un délai fixe ne suffit pas. `_DELAI_JUGEMENT_S` protège du re-bake, mais il DEVINE une durée :
***REMOVED*** le mur 906, plus lourdement habillé que celui sur lequel les 120 s ont été calibrées, était
***REMOVED*** encore à 17,7 ms bien après la fenêtre. Le tissu a jugé sur cette valeur transitoire et l'a
***REMOVED*** découpé en 2 shards — 9 cœurs et 3 processus au lieu de 3 — alors qu'en régime il tenait le
***REMOVED*** budget. On n'allonge pas le délai (on devinerait encore) : on exige que la saturation soit
***REMOVED*** CONSTATÉE PLUSIEURS FOIS DE SUITE. À un tick de 30 s, 4 passages = 2 min de saturation réelle.
_CONFIRMATIONS_SATURATION = 4

***REMOVED*** Et il faut une porte de SORTIE. Le critère de sortie du sharding était le nombre de tuiles
***REMOVED*** câblées, jamais le gain : un mur shardé par accident ne redevenait donc jamais monolithe. On
***REMOVED*** estime le coût qu'aurait le monolithe par la SOMME des `own` de ses shards — un MAJORANT (chaque
***REMOVED*** shard paie déjà son propre gather et sa propre sortie, que le monolithe ne paierait qu'une fois),
***REMOVED*** donc conclure « ça tient » sur cette base est conservateur. La marge crée l'HYSTÉRÉSIS qui évite
***REMOVED*** le flap : on shardE au-dessus du budget, on ne restaure qu'en dessous de 80 % de ce budget.
_MARGE_RESTAURATION = 0.80

_satur_streak = {}      ***REMOVED*** vmid → nb de passages CONSÉCUTIFS au-dessus du budget
_restau_streak = {}     ***REMOVED*** vmid → nb de passages CONSÉCUTIFS où le monolithe tiendrait
...
def _restaurables_tissu(node_id, budget_by_vmid, budget_ms):
    ...
    ***REMOVED*** ── ⛔ DÉSACTIVÉE PAR DÉFAUT — et ce n'est PAS de la prudence de principe ────────────────
    ***REMOVED*** La sortie économique est juste, mais elle est INEXPLOITABLE tant qu'un défaut amont n'est
    ***REMOVED*** pas corrigé : **restaurer un monolithe ne lui rend pas son GPU**. Mesuré le 2026-08-08 sur
    ***REMOVED*** le mur 906 — restauré, il est revenu en `gpu=False / mvk=True` (CPU), à 31,8 ms de `own`
    ***REMOVED*** pour un budget de 20, donc réellement saturé ; le tissu l'a re-shardé, à raison. On obtenait
    ***REMOVED*** une OSCILLATION : restauration → deux minutes de mur à 24-28 fps sous les yeux du
    ***REMOVED*** réalisateur → re-sharding. Pire que le défaut qu'on voulait corriger.
    ***REMOVED*** Réactiver seulement quand `restore_fn` réacquiert le GPU libéré au passage en assembleur
    ***REMOVED*** (le mur redevient alors ce qu'il était, pas une version CPU de lui-même).
        ...
        for sv in shards:
            ...
            if ip:
                try:
                    ...
                    lat = None
            if lat is None:
                complet = False       ***REMOVED*** mesure partielle → aucune conclusion
                break
            total += float(lat)
        if not complet:
...
def _reconcile_fabric_node_impl(node_id, perime=None):
    ...
    try:
        ...
        max_cells = 4
    ***REMOVED*** Exclure les NŒUDS INTERNES du tissu (shards/partagés) de l'ensemble à sharder : ce sont des
    ***REMOVED*** conteneurs de type `multiview`, mais les inclure ferait sharder les shards RÉCURSIVEMENT
    ***REMOVED*** (asm:<shard> + sous-shards dont le déploiement échoue → registre incohérent, shard reconfiguré
    ***REMOVED*** en assembleur lisant des fab inexistants → tuiles noires). Seuls les murs LOGIQUES sont candidats.
    from .database import db_fabric_all as _dfa
    _internal = set()
    ...
    ***REMOVED*** multiviews du nœud + leur own_latency (lecture :8080)
    ...
    for c in db_get_containers():
        ...
        if c.get("vmid") in _internal:
            continue   ***REMOVED*** shard/partagé du tissu → jamais re-shardé
        vmid = c.get("vmid"); p = dc.get("params") or {}
        mvs[vmid] = p
        ***REMOVED*** Intention de cadence PAR multiview (fps_target) → budget de trame propre ; sinon global.
        try:
            ...
            pass
        ***REMOVED*** ── ÉTAT TRANSITOIRE : ne pas juger un mur qui vient d'être déployé ─────────────────
        ***REMOVED*** Un (re)déploiement déclenche une rafale de re-bake — habillage plein cadre, frises,
        ***REMOVED*** vignettes — qui pousse la latence bien au-delà du budget pendant une à deux minutes,
        ***REMOVED*** AVANT que le régime établi ne s'installe. Le tissu, lui, réconcilie justement après un
        ***REMOVED*** déploiement : il mesurait donc la rafale et concluait « ce mur sature ».
        ***REMOVED*** Vécu le 2026-08-08 : un mur à 11 ms en régime, momentanément à 28 pendant sa rafale,
        ***REMOVED*** a été découpé en deux shards — fabriqués à partir du script en cours de déploiement,
        ***REMOVED*** qui se trouvait être cassé. Le tissu a ainsi propagé une panne au lieu de la contenir.
        ***REMOVED*** On IGNORE donc les murs déployés depuis moins de _DELAI_JUGEMENT_S : latence None =
        ***REMOVED*** non jugé, ni shardé ni restauré. Le prochain cycle les prendra en régime établi.
        ...
        if _dep:
            try:
                ...
                if 0 <= _age < _DELAI_JUGEMENT_S:
                    latency[vmid] = None
                    _satur_streak[vmid] = 0     ***REMOVED*** la fenêtre de rafale ne compte pour rien
        ...
        if ip:
            try:
                ...
                lat = None
        ***REMOVED*** ── SATURATION CONFIRMÉE, jamais sur un seul relevé ─────────────────────────────────
        ***REMOVED*** Le délai de rafale devine une durée ; ceci constate un RÉGIME. Un mur au-dessus du
        ***REMOVED*** budget une seule fois n'est pas saturé — il peut sortir d'un re-bake, d'un pic de
        ***REMOVED*** frise, ou d'un démarrage plus lent que la fenêtre calibrée. Tant que la saturation
        ***REMOVED*** n'est pas confirmée, on rend `None` : non jugé, ni shardé ni restauré (même contrat
        ***REMOVED*** que la fenêtre de rafale). Découper un mur coûte 6 cœurs et 2 conteneurs, ça vaut
        ***REMOVED*** bien deux minutes de patience.
    ...
    def _deploy(name, params, hostname):
        ***REMOVED*** crée un conteneur compute (multiview shard) + déploie son script ; renvoie str(vmid) = ref.
    ...
    def _apply_mv(ip, default_template, overlays, flux_config, meter_blocks=None,
        ...
        ***REMOVED*** Applique style (/style) + overlays (/overlays) + sources+meter_blocks (/reconfigure) À
        ***REMOVED*** CHAUD. default_template : le MODÈLE PAR DÉFAUT du mur suit le RÔLE du conteneur — None
        ***REMOVED*** pour l'ASSEMBLEUR (copie pure : sans le clear explicite, le DEFAULT_TEMPLATE du mur
        ***REMOVED*** restait chargé dans le moteur converti à chaud → l'habillage se redessinait autour de
        ***REMOVED*** chaque BLOC de shard), la valeur du mur au restore (monolithe).
        ***REMOVED*** meter_blocks : VU-mètres de MUR (0.36.0) — même remplacement atomique que flux_config,
        ***REMOVED*** aucune résolution de dims (cf. _multiview_hot_apply). Toujours réappliqués (assembleur
        ***REMOVED*** ET restore) car ils étaient jusqu'ici silencieusement perdus par le canal fabric (le
        ***REMOVED*** mur logique édité persistait bien meter_blocks en DB — cf. _multiview_hot_apply — mais
        ***REMOVED*** ce canal-ci, dédié aux transitions shard↔monolithe, ne les propageait jamais au conteneur).
        ***REMOVED*** Polices : ce canal-ci (transitions shard↔monolithe) reconfigure À CHAUD un conteneur
        ***REMOVED*** dont le script a pu être rendu AVANT l'ajout de la police → on ré-embarque le base64
        ***REMOVED*** des polices référencées par les params poussés (sinon un mur shardé retomberait sur
        ***REMOVED*** DejaVu). Résolution à la volée : rien de gras n'est jamais persisté (cf. _gras).
        ...
        ***REMOVED*** Statuts VÉRIFIÉS et renvoyés : ce canal sert aussi au REBIND d'un shard, où « le push
        ***REMOVED*** a-t-il pris ? » décide de garder le conteneur ou de le remplacer. Un 500 avalé y
        ***REMOVED*** laisserait un shard au contenu périmé, sans rien de visible côté orchestrateur.
        ...
        _r3 = requests.post(f"http://{ip}:8082/reconfigure",
                            json={"flux_config": flux_config or [], "meter_blocks": meter_blocks or [],
                                  ***REMOVED*** Frises d'historique de MUR (0.37.0) : même transit que meter_blocks
                                  ***REMOVED*** (sinon même fuite silencieuse sur un mur shardé).
                                  "video_history_blocks": video_history_blocks or [],
        ...
        while attente and (_time.monotonic() - t0) < deadline_s:
            ...
            for r in sorted(attente):
                try:
                    ...
                    attente.discard(r); continue      ***REMOVED*** ref non numérique : pas d'attente possible
                if not ip:
                    continue
                try:
                    ...
                    continue
                ***REMOVED*** Un agent qui répond ne suffit PAS : le flux MXL peut ne pas encore exister. Le
                ***REMOVED*** témoin est la CADENCE publiée — le multiview ne sert pas de `frame_index` (il
                ***REMOVED*** publie `fps`/`frames_per_s`) ; on tolère `frame_index` pour les autres types.
                ***REMOVED*** ⚠ Ne jamais deviner ce nom : s'être fié au contrat générique de CLAUDE.md sans
                ***REMOVED*** vérifier ce plugin a figé toutes les bascules du tissu (2026-08-06).
    ...
    def _rebind(ref, params):
        ...
        if not ip:
            return False
        ***REMOVED*** Même règle que la garde de hot-apply : ce que le shard EXÉCUTE prime sur l'estampille
        ***REMOVED*** en base. Muter à chaud un script d'une autre version, c'est le risque de champs
        ***REMOVED*** récents silencieusement ignorés — le refus fait retomber sur créer+détruire, qui
        ***REMOVED*** repose un script neuf.
    ...
    def _restore(vmid, logical):
        ...
        if not ip:
            return
        try:   ***REMOVED*** restaure le modèle par défaut + overlays + sources + meter_blocks d'ORIGINE (monolithe)
    ...
    ***REMOVED*** TISSU EN TRANCHES (docs/chantiers/TISSU_SLICE.md) : setting fabric_slice_mode (défaut OFF, opt-in acté)
    ***REMOVED*** OU réglage global slice_mode_global (Réglages → Vidéo — l'un OU l'autre actif ⇒ tissu en
    ***REMOVED*** tranche) → les nœuds du tissu sont matérialisés en cadence "flow" + slice. NB : un nœud DÉJÀ
    ***REMOVED*** matérialisé garde ses params (dédup par signature) — basculer le setting exige un teardown
    ***REMOVED*** des nœuds fab (ou l'édition du mur, qui re-planifie) pour prendre effet.
    _slice = any(str(_st.get(k) or "").strip().lower() in ("1", "true", "yes", "on")
                 for k in ("fabric_slice_mode", "slice_mode_global"))
    ***REMOVED*** Combien de shards ce nœud peut-il réellement ÉPINGLER ? Budget = cœurs LIBRES du pool +
    ***REMOVED*** ceux que tiennent déjà les shards du tissu (ils seront relâchés si la découpe change) ÷
    ***REMOVED*** profil du type. Sans ce plafond, le tissu planifiait des conteneurs que le nœud ne pouvait
    ***REMOVED*** que sur-souscrire — mesuré sur dl360-1 : `0 cœur(s) libre(s)/12, 3 demandé(s) → repli quota`.
    _max_noeuds = None
    try:
    ...
    return _cf.reconcile_fabric(node_id, mvs, latency, _deploy, _destroy, _reconfigure, _restore,
                                ...
                                perime_fn=perime,
                                ***REMOVED*** Sortie ÉCONOMIQUE du sharding : murs dont les shards cumulés
                                ***REMOVED*** tiendraient dans le budget d'un monolithe, confirmé plusieurs
                                ***REMOVED*** passages. Sans ça, shardé un jour = shardé toujours.
