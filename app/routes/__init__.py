...
@require_global_access
def export_alertes():
    """Export des alertes — POINT D'ENTRÉE HISTORIQUE, conservé.

    Les boutons de `monitoring.html` et de `scripts.js` pointent dessus : c'est un contrat, on
    ne le renomme pas. Il DÉLÈGUE désormais la sérialisation CSV à `journaux_export`, qui a
    corrigé trois défauts de la version d'origine :
      · pas de BOM UTF-8 → Excel en locale française lisait du Latin-1 et cassait les accents ;
      · virgule comme séparateur → mêmes tableurs, tout arrivait dans une seule colonne ;
      · aucune neutralisation d'INJECTION CSV — un message d'alerte commençant par `=`, `+`,
        `-` ou `@` est interprété comme une FORMULE par le tableur qui l'ouvre, et les messages
        d'alerte contiennent du texte d'origine externe (noms de flux, détails d'erreur). Sur
        un fichier qu'on envoie à un client, c'était un vrai trou.
    Les colonnes de contexte (`nature`, `vmid`, `noeud_id`) sont AJOUTÉES en fin de ligne : un
    import existant qui lit les trois premières colonnes continue de fonctionner.

    `format=txt` est conservé tel quel : c'est une sortie de lecture, pas d'import."""
    from flask import Response
    from ..i18n import rendre_alertes
    from .journaux_export import COLONNES, _csv
    q = (request.args.get("q") or "").strip() or None
    niveau = (request.args.get("niveau") or "").strip() or None
    ...
    corps = "\ufeff" + _csv(rows, COLONNES["alertes"], sep)
    return Response(corps, mimetype="text/csv; charset=utf-8",
        headers={"Content-Disposition": f'attachment; filename="alerts-{stamp}.csv"',
                 "X-Lignes": str(len(rows))})
