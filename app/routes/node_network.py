...
def api_node_interface_set(node_id, ifname):
    ...
    # ★ ENREGISTRÉ ≠ APPLIQUÉ. Un moteur 2110 ne relit sa configuration réseau qu'au DÉPLOIEMENT :
    # l'adressage et l'appariement red/blue posés ici ne prennent effet qu'après. Sans ce retour,
    # l'opérateur voit « interface enregistrée », le croit à raison, et n'a aucun moyen de savoir
    # que le moteur tourne encore sur l'ancienne — c'est exactement la question posée en
    # exploitation le 2026-09-10. Et comme armer une paire demande DEUX éditions (le rouge puis le
    # bleu), on ne redéploie pas à chaque enregistrement : on signale, et l'opérateur applique une
    # fois qu'il a fini.
        ...
        for _c in db_get_containers():
            ...
            if derive_config_moteur(_c["vmid"]):
                moteurs_a_redeployer.append({"vmid": _c["vmid"],
                                             "hostname": _c.get("hostname") or "#%s" % _c["vmid"]})
