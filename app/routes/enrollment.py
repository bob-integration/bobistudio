...
***REMOVED*** ─── Enrôlement zéro-touch (clé USB préseedée) ─────────────────────────────────
def _enroll_finalize(node_id, ip):
    ...
    up = False
    for _ in range(180):                     ***REMOVED*** ~30 min : le 1er boot installe tout
    ...
    ***REMOVED*** ★ PRÉFIXE DE LIBELLÉS SEMÉ PAR NŒUD (BCP-002-01, 2026-08-22). Sans override, tous les nœuds
    ***REMOVED*** partagent le littéral « 2110 » : deux moteurs 2110 sur deux nœuds émettent alors le même
    ***REMOVED*** couple groupe:rôle sous un Device cluster UNIQUE, et le MUST d'unicité du rôle dans un
    ***REMOVED*** groupe tombe — un contrôleur tiers fusionne les Rx des deux nœuds. Semer ici corrige la
    ***REMOVED*** CAUSE ; le garde-fou d'écriture (db_nmos_resource_upsert) ne rattrape que le symptôme.
    ***REMOVED*** Semé UNE FOIS et seulement si rien n'est posé : l'exploitant reste maître du libellé.
    try:
    ...
    ***REMOVED*** Provisionne les images PARTAGÉES (compute/media/webrtc) requises par les capacités, depuis
    ***REMOVED*** l'HÔTE DE BUILD (local OU export depuis le nœud de build) — pas seulement le docker local de
    ***REMOVED*** l'orchestrateur. `mtl` se build par-nœud (carte du nœud), pas poussé ici.
    try:
    ...
    ***REMOVED*** Pool de cœurs : posé DÈS L'ENRÔLEMENT, sinon un nœud neuf ne peut rien réserver — ses
    ***REMOVED*** conteneurs restent tous partagés et le bouton « Garantir » se refuse, sans que rien n'indique
    ***REMOVED*** qu'il manque un réglage. Constaté le 2026-08-02 : cinq nœuds sur six sans pool.
    ***REMOVED*** Dérivé selon la NATURE du nœud (cf. core_pool.pool_par_defaut) — donc APRÈS `register`, qui
    ***REMOVED*** écrit les capacités en base.
    if ok:
        try:
    ...
    ***REMOVED*** Bascule mTLS automatique une fois le nœud enregistré (si la CA interne est initialisée).
    ***REMOVED*** rotate_tls : init CSR → signe → install → l'agent redémarre en HTTPS. Filet interne (reste en
    ***REMOVED*** HTTP si un pas échoue). Séquentiel après register → aucune course avec les sondes HTTP ci-dessus.
    from .. import ca
    if ok and ca.ca_available():
        try:
            node = db_get_node(node_id)
            ***REMOVED*** `rotate_tls` retourne un TUPLE (ok, msg) : le tester directement rendait la condition
            ***REMOVED*** TOUJOURS vraie (un tuple non vide est truthy) et l'enrôlement annonçait « mTLS activé »
            ***REMOVED*** même quand la bascule avait échoué et que le nœud restait en HTTP. C'est ainsi que
            ***REMOVED*** dl360-1 et dell-1 sont restés sans certificat, enrôlés APRÈS l'automatisation, sans
            ***REMOVED*** que rien ne le dise. Le journal affirmait le contraire de la base.
...
def api_node_profile():
    ...
    ***REMOVED*** MAC du port de gestion (multi-NIC) : épingle d-i sur la bonne carte à l'install. Optionnel.
    ...
    profile = {
        ...
        ***REMOVED*** Plan de CONTRÔLE : pour mémoire/affichage (l'IP réelle reste apprise du remote_addr à
        ***REMOVED*** l'enrôlement). Vide = DHCP ; sinon IP statique gravée dans le preseed via build-node-iso.sh.
...
def api_node_enroll():
    ...
    facts = request.json or {}            ***REMOVED*** faits matériels postés par node-bootstrap.sh (nics/ice/mac)
    try:
        prof = _json.loads(node.get("enroll_profile") or "{}")
    except Exception:
        prof = {}
    ***REMOVED*** host = IP source du nœud ; consomme le jeton (one-time) ; passe en 'enrolling'.
    ***REMOVED*** ★ Clé d'hôte SSH : une machine RÉINSTALLÉE présente une nouvelle clé pour la même IP. Le
    ***REMOVED*** `known_hosts` du contrôleur porte encore l'ancienne → « HOST IDENTIFICATION HAS CHANGED » et
    ***REMOVED*** TOUT chemin ssh du contrôleur vers ce nœud échoue durement (host_ops, services/files,
    ***REMOVED*** media_manager, build/inspect d'images), sans rapport apparent avec l'installation. Un
    ***REMOVED*** enrôlement EST le moment légitime pour oublier l'ancienne identité de cette adresse.
    if ip:
        try:
    ...
    ***REMOVED*** Capture l'inventaire matériel pour la config réseau POST-enrôlement (choix du parent macvlan
    ***REMOVED*** depuis l'UI). ice_iface (E810) → mtl_iface direct ; nics/mac mémorisés dans le profil.
    ...
    ***REMOVED*** Le jeton est consommé → l'URL ISO iLO expire ; purge l'ISO cachée (cf. node_iso).
    try:
        from .. import node_iso
        node_iso.purge(token)
    except Exception:
        pass
    ***REMOVED*** mTLS : l'enrôlement se fait TOUJOURS en HTTP (le nœud n'a pas encore de cert). La bascule
    ***REMOVED*** HTTPS est déclenchée par le contrôleur À LA FIN de _enroll_finalize (rotate_tls), une fois le
    ***REMOVED*** nœud enregistré — séquentiel, donc pas de course de timing avec les sondes HTTP de finalize.
    ***REMOVED*** (On ne signe PAS de cert ici : un cert renvoyé à l'enroll ferait basculer le nœud en HTTPS
    ***REMOVED*** pendant que finalize sonde encore en HTTP → nœud faussement « down ».)
    ...
    return jsonify({
        ...
        ***REMOVED*** Tags d'images ATTENDUS, dérivés des meta.json (source unique, cf. images._image_tag) et
        ***REMOVED*** filtrés par les capacités du profil. Sans ça l'installeur retombait sur des tags codés en
        ***REMOVED*** dur et périmés (bobi-compute:0.2 face à une flotte en 0.29) : faux « image ABSENTE » à
        ***REMOVED*** chaque install, tags périmés persistés dans config.json, et — avec un registry — un pull
        ***REMOVED*** RÉEL d'une version périmée annoncée comme bonne.
        "images": _images_attendues(prof.get("capabilities") or []),
        "agent_port": prof.get("agent_port") or 9100,
        ***REMOVED*** Clé publique du contrôleur → authorized_keys du nœud (SSH root d'ops, le bootstrap l'installe).
        "controller_ssh_key": host_ops.controller_ssh_pubkey(),
        ***REMOVED*** Noyau compatible MTL (io2110) : réglage CLUSTER, non figé. Vide = aucun épinglage (avertissement).
...
***REMOVED*** ─── Gravure de la clé USB d'enrôlement DEPUIS le contrôleur ───────────────────
***REMOVED*** Détecte les clés amovibles branchées sur l'hôte du contrôleur, construit l'ISO préseedée et la
***REMOVED*** grave (dd). Destructif → garde-fous dans app/usb_flash (amovible only, disque système exclu, device
***REMOVED*** re-validé serveur). Alternative à la commande build-node-iso.sh à taper.
...
***REMOVED*** ─── Servir l'ISO d'enrôlement à iLO 5 (Virtual Media URL) + montage auto Redfish ───────────────
***REMOVED*** Au lieu de graver une clé USB, on CONSERVE l'ISO préseedée par-nœud et on l'expose en HTTP à une
***REMOVED*** URL gardée par le enroll_token (cf. app/node_iso, app/ilo). iLO la monte en CD/DVD virtuel.

***REMOVED*** Route PUBLIQUE (sans auth : iLO n'envoie aucun cookie de session) — gardée par le enroll_token
***REMOVED*** one-time non devinable, dans le chemin. 404 dès que le token est consommé (URL expire d'elle-même).
***REMOVED*** send_file(conditional=True) → Werkzeug honore les requêtes Range d'iLO (lecture partielle du CD).
...
def api_node_ilo_creds(node_id):
    ...
    if pwd:   ***REMOVED*** ne mettre à jour le mot de passe que s'il est fourni non vide
...
def api_node_enroll_config(node_id):
    ...
    try:
        ...
        prof = {}
    ***REMOVED*** Réseau de contrôle (IP statique / DHCP) — réglé à l'étape 2 (détail du jeton).
    if "mgmt_ip" in d:
    ...
    if "partition" in d:
        ...
        if part:
            ...
            prof.pop("partition", None)   ***REMOVED*** désélection
...
***REMOVED*** ─── Boot réseau PXE / UEFI HTTP Boot (Phase 0, sans licence iLO) ───────────────
***REMOVED*** Le contrôleur sert l'arbre netboot Debian + grub.cfg/preseed/payload générés en HTTP. Un nœud HPe
***REMOVED*** Gen10 boote dessus via RBSU → UEFI HTTP Boot. Cf. app/pxe.py + PXE_ANALYSIS.md.

***REMOVED*** Route PUBLIQUE (le firmware/installeur n'authentifie pas). Sert l'arbre netboot, MAIS intercepte
***REMOVED*** grub.cfg / preseed.cfg / payload/* qui sont GÉNÉRÉS pour le nœud armé. Le seul secret (enroll_token,
***REMOVED*** dans enroll.conf) est one-time, consommé à l'enrôlement — même modèle que la clé USB / l'ISO iLO.
...
def serve_pxe(filename):
    ...
    ***REMOVED*** grub.cfg (quel que soit le sous-chemin demandé par bootnetx64.efi selon son prefix compilé)
    if base == "grub.cfg":
        if not ctrl:
            abort(404)
        node = db_get_node_by_enroll_token(token) if token else None
    ...
    ***REMOVED*** Sinon : fichier statique de l'arbre netboot (noyau/initrd/efi…), avec garde anti-traversal.
    ***REMOVED*** Content-Type FORCÉ à application/octet-stream : le firmware UEFI HTTP Boot (HPe Gen10/iLO5)
    ***REMOVED*** exige ce type pour le NBP (bootnetx64.efi) et abandonne silencieusement sur tout autre type
    ***REMOVED*** (ex. application/efi déduit de l'extension) → écran « booting URL file » figé après le HEAD.
...
def api_pxe_state():
    ...
    token, ctrl = pxe.armed()
    node = db_get_node_by_enroll_token(token) if token else None
    ***REMOVED*** Mot de passe root du nœud armé : il n'existe nulle part ailleurs, et l'opérateur en a besoin
    ***REMOVED*** pour la console iLO si le réseau tombe pendant l'installation. Effacé au désarmement.
    return jsonify({"ready": pxe.netboot_ready(),
                    "armed_node_id": (node or {}).get("id"),
                    "armed_node_name": (node or {}).get("name"),
                    "root_password": pxe.mdp_root_arme(),
