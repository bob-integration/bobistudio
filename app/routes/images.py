...
# Chemin de données du plugin decklink_io, compilé dans l'image compute (étage decklink-builder).
# Les en-têtes du SDK sont un DOSSIER : le garde-fou de COPY ci-dessous les vérifie comme tel.
...
def _image_tag(which):
    ...
    spec = _IMAGES[which]
    # ⚠ LE `try` NE COUVRE QUE LA LECTURE. Y laisser le contrôle de cohérence le rendait MUET :
    # l'exception était avalée et la fonction rendait `<prefix>:latest` — donc on aurait bâti
    # sous un tag encore différent, en silence, au lieu de refuser. Vérifié par mutation, le
    # premier jet faisait exactement ça.
    try:
    ...
    if tag and ver:
        # On compare sur les segments COMMUNS : `version` est en trois segments (0.46.0), le tag
        # en deux (bobi-compute:0.46). Comparer les chaînes entières refuserait tout.
...
def _build_image_worker(which):
    ...
    # ⚠ CE THREAD EST LE SEUL À POUVOIR SORTIR `_img_build` DE L'ÉTAT « building ». S'il meurt
    # sur une exception, l'écran reste à « en cours » indéfiniment et personne ne sait pourquoi
    # — un refus qui se présente comme une attente est pire qu'un refus. `_image_tag` LÈVE
    # désormais sur un meta.json incohérent : il faut donc le dire.
    try:
        tag = _image_tag(which)
    except Exception as e:
        with _img_lock:
    ...
    try:
        ...
        if rc == 0 and present:
            ...
            # ★ SECOND PASSAGE, APRÈS LA DISTRIBUTION (2026-09-08). Le premier ne pouvait
            # renseigner QUE le nœud de build : à ce moment-là, les bits ne sont arrivés nulle
            # part ailleurs, et la garde de 2026-08-21 refuse — à raison — d'inscrire un tag
            # qu'elle n'a pas vu sur le nœud. Conséquence, mesurée le 2026-09-08 : après un build
            # réussi ET distribué à 3 nœuds, un seul avait le nouveau tag en base ; les autres
            # continuaient d'exécuter l'ANCIENNE image, avec les bits neufs sur leur disque. Il
            # fallait un DEUXIÈME build du même tag pour que la flotte l'adopte — et rien ne le
            # disait. La garde était juste, c'est le moment de l'appel qui ne l'était pas.
            filled += _autofill_nodes_image(which, tag)
