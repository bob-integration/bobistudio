...
def _fmt_from_sdp(sdp):
    ...
    ***REMOVED*** Scan EXPLICITE depuis le SDP : « interlace » présent ⇒ i, sinon p. On le pose toujours (≠ None)
    ***REMOVED*** pour que la topologie/Sources ne retombe PAS sur le scan CONFIGURÉ (dc_params) d'un flux actif
    ***REMOVED*** (un flux 1080p s'affichait « 1080i » à cause du repli sur la config).
    if mw or mh:
        out["scan"] = "i" if _re.search(r"\binterlace\b", sdp) else "p"
    ***REMOVED*** Reste du fmtp ST 2110-20 : chroma (sampling), profondeur, colorimétrie, transfert (HDR), range.
...
def _io_media_ports(node, params, pins_key):
    ...
    ***REMOVED*** Résolution slot→port : docker_driver.engine_slot_ports est LA source unique (miroir du
    ***REMOVED*** controller). Cette vue n'ajoute que l'habillage humain (réseau, alias, drapeau `pinned`).
    ...
    ***REMOVED*** Alias humain par interface (node_interfaces.alias, ex. « PGM-Rouge ») — clé = ifname (netdev).
    _alias_by_if = {}
...
def _compute_receivers_detail(only_vmid=None):
    ...
    by_container = {}  ***REMOVED*** vmid → list de receivers
    for r in recv_snapshot:
        ***REMOVED*** tag vmid absent/« None » (receiver mal enregistré) → on ignore au lieu de 500 (cf. senders).
    ...
    for vmid, recvs in sorted(by_container.items()):
        ...
        ***REMOVED*** deploy_config → état du générateur par slot
        ...
        ***REMOVED*** Multi-NIC : ports média du nœud + assignation effective par slot (auto modulo / épinglé).
        ***REMOVED*** Calculé seulement si le nœud a ≥2 ports (sinon mono-port → pas de sélecteur côté UI).
        ...
        ***REMOVED*** Association audio→vidéo : audio_count = video_count × audio_per_video, les audios
        ***REMOVED*** étant rangés par blocs (vidéo 0 : audios 0..aper-1, vidéo 1 : aper..2aper-1, …).
        ***REMOVED*** Fallback si audio_per_video absent (vieux containers) : division entière exacte.
        ...
        ***REMOVED*** Groupement audio/ANC→vidéo (« Option A ») : piloté par rx_flows (attached_to) ; repli
        ***REMOVED*** dérivé du legacy si la liste est absente. Cartes (essence, idx) → idx vidéo / rang.
        ...
        ***REMOVED*** Fetch live fps par pipeline (clé (essence, idx) — vidéo et audio peuvent partager un idx)
        per_key_fps = {}
        nic_info = {}
        data = {}   ***REMOVED*** réponse :8080 — initialisée AVANT le fetch : si le container est injoignable
                    ***REMOVED*** (down, IP changée, timeout), on dégrade proprement (xdp/nic vides) au lieu de
                    ***REMOVED*** lever un NameError plus bas sur data.get("xdp") → 500 qui casse Sources + Destinations.
        if ip:
            try:
        ...
        for r in sorted(recvs, key=_sort_recv):
            ...
            ***REMOVED*** ⚠ NOM DE FLUX = `numerotation` (2026-08-19) : construit ici sur l'indice BRUT, il
            ***REMOVED*** restait 0-based après la migration du 2026-08-13 et désignait donc le flux VOISIN.
            ...
            ***REMOVED*** Détails du générateur local (pour le tooltip riche côté front).
            ***REMOVED*** Exposé même éteint : le tooltip montre alors les réglages mémorisés.
            if essence == "audio":
                gen = {
            ...
            ***REMOVED*** Format affiché : si un SDP est actif, on le lit du SDP (le receiver s'adapte au
            ***REMOVED*** flux reçu) ; sinon repli sur le deploy_config (format configuré). Vaut pour tous
            ***REMOVED*** les receivers (LXC ffmpeg comme MTL) — la source de vérité d'un flux actif = son SDP.
            ...
            receivers_out.append({
                ...
                ***REMOVED*** État LIVE du moteur (:8080) : init|mtl|simu|idle|error. `generating` = une mire
                ***REMOVED*** tourne RÉELLEMENT sur ce slot vidéo (gen explicite) → badge GÉN honnête côté UI
                ***REMOVED*** (≠ `simulated` qui dérive de la config). Un slot non abonné sans gen = "idle".
                "mode": live.get("mode"),
                "generating": (essence == "video" and live.get("mode") == "simu"),
                ***REMOVED*** « Abonné mais ne reçoit pas » (vidéo) : mode mtl mais flux figé (échec création
                ***REMOVED*** st20p_rx_create / budget lcores, OU pas de trafic réseau). Surfacé en badge.
                "rx_stalled": (bool(_rx_stalled_d.get(vmid, {}).get(shm_name)) if essence == "video" else None),
                ***REMOVED*** Timecode ATC (essence ANC) : décodé par mtl_rx, relayé via les métriques :8080.
                "timecode": (live.get("timecode") if essence == "anc" else None),
                "df":       (bool(live.get("df")) if essence == "anc" else None),
                ***REMOVED*** Format vidéo : SDP actif prioritaire, repli deploy_config.
                ...
                ***REMOVED*** Chroma / colorimétrie / transfert(HDR) / range VIDÉO — SDP prioritaire, repli config.
                ...
                ***REMOVED*** Format AUDIO (2110-30) lu du SDP actif : fréquence / nb de canaux.
                "sample_rate": (_afmt.get("sample_rate") if essence == "audio" else None),
                "channels":    (_afmt.get("channels")    if essence == "audio" else None),
                ***REMOVED*** Profondeur de bits : audio (L24…) depuis le SDP audio ; vidéo (depth=) SDP→config.
                ...
                ***REMOVED*** IDENT (incrustation 3 lignes) — slots vidéo uniquement.
                ...
                ***REMOVED*** SDP actif (transport_file) : vidéo ET audio. La modale d'édition reste
                ***REMOVED*** vidéo-only côté UI, mais l'audio en a besoin pour afficher son multicast.
                "sdp": (((state_snapshot.get(rid, {}).get("active") or {})
                         .get("transport_file") or {}).get("data")),
                ***REMOVED*** Association audio→vidéo (groupement + nommage « Audio v-a ») — via rx_flows
                ***REMOVED*** (attached_to). None = flux INDÉPENDANT (non rattaché à une vidéo).
                ...
                ***REMOVED*** Identité du flux composable (« Option A ») — pour add/remove granulaire côté UI.
                "flow_id":       _rx_id_of.get((essence, idx)),
                ***REMOVED*** Port (NIC) effectif du slot RX (multi-NIC) : {iface, pinned} ou None si mono-port.
                "port":          _slot_port(idx),
            })

        ***REMOVED*** Estimation théorique du débit RX (si pas de stats live du container)
        ...
        if nic_rx_gbps is None and n_video_cfg:
            ...
            nic_rx_estimated = None
        ***REMOVED*** PLANIFIÉ = flux provisionnés (RX + TX) — chacun = 1 file AF-XDP quand il deviendra LIVE
        ***REMOVED*** (abonné/câblé). `xdp_active` (live) ≤ `xdp_planned` ≤ pool. Permet à la barre B2 de montrer
        ***REMOVED*** une zone « planifié » qui réagit aux ajouts/retraits de sources/destinations (en simu, un
        ***REMOVED*** flux ne consomme pas encore de file → seul le planifié bouge, l'actif suit à l'abonnement).
        from .. import io2110_flows as _iof
        _xdp_planned = len(_iof.active_flows(dc_params, "rx")) + len(_iof.active_flows(dc_params, "tx"))
        ***REMOVED*** Stats PAR PORT physique (multi-NIC) : fusion des descripteurs moteur (nic.ports : débit
        ***REMOVED*** mesuré / files / lien) + métadonnées nœud (nom de réseau → couleur red/blue UI) + comptage
        ***REMOVED*** de flux et CHARGE ESTIMÉE par port (repli quand ethtool ne mesure rien sur le port). Mono-
        ***REMOVED*** port → [] : l'UI retombe sur la tuile agrégée existante (pas de régression).
        nic_ports_out = []
        if _multi:
            ...
            ***REMOVED*** Planifié PAR PORT : flux provisionnés (rx+tx) → port (assignation déterministe modulo/
            ***REMOVED*** épinglage). C'est un COMPTE DE FLUX exact par port (comme le planifié global) ; le LIVE
            ***REMOVED*** par port, lui, vient du moteur (sessions, fan-out compris) — d'où nic.ports[].active.
            _planned_per = {}
            ...
            for _f in _iof.active_flows(dc_params, "tx"):
                ...
                if _p: _planned_per[_p] = _planned_per.get(_p, 0) + 1
            ***REMOVED*** État PTP par port (cache sampler, pas de SSH) : SLAVE|MASTER|PASSIVE|LISTENING|FAULTY…
            _ptp_states = {}
            try:
            ...
            for _pm in _ports_meta:
                ...
                nic_ports_out.append({
                    ...
                    ***REMOVED*** Files AF-XDP PAR PORT (barre XDP par NIC) : live=sessions du moteur (exact),
                    ***REMOVED*** réservé=allocation mtl_init du port, planifié=flux provisionnés, plafond=HW du port.
                    "xdp_active":          _ep.get("active"),
                    "xdp_reserved":        ((_rxq or 0) + (_txq or 0)) or None,
                    "xdp_planned":         _planned_per.get(_if, 0),
                    ***REMOVED*** Plafond HW du port : per-port (moteur ≥0.34) sinon repli sur le global (même carte).
                    "xdp_hw":              _ep.get("hw_max_combined") or (data.get("xdp") or {}).get("hw_max_combined"),
                    "ptp_state":           _ptp_states.get(_if),
                    ***REMOVED*** Socle DPDK narrow (moteur ≥0.39.16, ports pmd=dpdk) : sessions RL TX live /
                    ***REMOVED*** cap RL du port (la limite dure — docs/chantiers/DPDK_NARROW.md §7) + sessions RX (RSS).
                    ***REMOVED*** None/absent sur un port af_xdp → l'UI garde la barre « Queues XDP ».
                    "pmd":                 _ep.get("pmd"),
                    "rl_tx_cap":           _ep.get("rl_tx_cap"),
                    "tx_sessions_active":  _ep.get("tx_sessions_active"),
                    "rx_sessions_active":  _ep.get("rx_sessions_active"),
                })
        ***REMOVED*** Modèle + agrégat NIC : on PRÉFÈRE les sources fiables de l'orchestrateur au modèle brut du
        ***REMOVED*** moteur (heuristique sysfs → « E810 QSFP » + 100G en dur, faux pour une 4-ports SFP).
        ***REMOVED***  - modèle : nom exact résolu par lspci, persisté dans node_interfaces (config réseau).
        ***REMOVED***  - agrégat : SOMME des vitesses de lien réelles des ports de la carte média (4×10 = 40G),
        ***REMOVED***    via les ports du moteur (live) sinon les vitesses DB. Repli sur la valeur moteur.
        ...
        ***REMOVED*** SMPTE 2022-7 « pair-aware » : une paire red/blue = UNE capacité UTILE (le second
        ***REMOVED*** port duplique le trafic, il n'ajoute rien). Capacité = min de la paire ; usage
        ***REMOVED*** utile = max de la paire (les deux legs portent les mêmes flux). Sans 2022-7 :
        ***REMOVED*** sommes classiques (aucune régression multi-NIC non redondant).
        ...
        ***REMOVED*** Allège les préfixes génériques lspci (« Ethernet Network Adapter E810-XXV-4 » → « E810-XXV-4 »).
        ...
        ***REMOVED*** Plafond HW XDP agrégé = SOMME des max combined par port (chaque PF E810 a son
        ***REMOVED*** budget propre) — pair-aware en 2022-7 (les files d'une paire portent les MÊMES
        ***REMOVED*** sessions). Repli mono-port (nic_ports_out vide) sur la valeur per-port du moteur.
        ...
        ***REMOVED*** Débits UTILES : en 2022-7 le trafic est dupliqué sur les deux legs → l'usage
        ***REMOVED*** d'une paire = max des deux ports (pas la somme).
        if _is227 and nic_ports_out:
        ...
        ***REMOVED*** ── Supervision RL (socle DPDK narrow) : bloc `rl` live du moteur (≥0.39.16) + repli
        ***REMOVED*** orchestrateur quand le moteur est muet (down / image antérieure) mais que le nœud est
        ***REMOVED*** en PF dpdk + pacing RL : cap RL par port via profil mesuré (nic_profiles) / bibliothèque
        ***REMOVED*** de cartes (_node_rl_tx_cap) — cf. _mtl_rl_tx_budget (même logique de repli).
        ...
        out.append({
            ...
            ***REMOVED*** Ports média du nœud (multi-NIC) : [] si mono-port → pas de sélecteur côté UI.
            "ports": _ports_meta,
            ***REMOVED*** SMPTE 2022-7 : paires red/blue actives → l'UI affiche/sélectionne des PAIRES.
            "smpte_2022_7": _is227,
            "port_pairs": [list(p) for p in _pairs227] if _is227 else [],
            ***REMOVED*** Stats ventilées par port physique (multi-NIC) : [] si mono-port.
            "nic_ports": nic_ports_out,
            ***REMOVED*** Capacité totale déployée vs slots visibles dans NMOS (pour le bouton « + »)
            "video_count":      n_video_cfg,
            "active_rx_count":  min(int(dc_params.get("active_rx_count") or n_video_cfg), n_video_cfg),
            ***REMOVED*** Stats NIC live (container up) ou estimées (container down)
            ...
            ***REMOVED*** Queues AF_XDP : sessions actives (live) / réservation mtl_init (plafond à chaud) / HW NIC.
            ***REMOVED*** `reserved` = files figées au dernier lancement (au-delà → redéploiement requis) ; `allocated`
            ***REMOVED*** suit la demande courante (legacy, conservé pour repli image pré-A2).
            ...
            ***REMOVED*** Plafond HW : SOMME sur les 4 ports (chaque port E810 a son propre budget combined, ex.
            ***REMOVED*** 4×96=384) — `nic_ports_out[].xdp_hw` est le max PAR PORT. Le bloc `xdp` brut du moteur ne
            ***REMOVED*** lit que le port primaire (96), incomparable à `allocated`/`reserved` qui sont déjà des
            ***REMOVED*** sommes 4 ports → on agrège ici. Repli mono-port (nic_ports_out vide) : valeur moteur.
            "xdp_hw_max_combined":     _xdp_hw_total,
            "xdp_hw_current_combined": _xdp_hw_total,   ***REMOVED*** current==max sur E810 (tous canaux activés)
            "xdp_hw_xdp_available":    (None if _xdp_hw_total is None
                                        else max(0, _xdp_hw_total - ((data.get("xdp") or {}).get("allocated") or 0))),
            ***REMOVED*** Plafond combined PAR PORT (≠ agrégat ci-dessus) : utilisé par le calcul de budget de files
            ***REMOVED*** `_mtl_total_queues(...) × nb_ports`. Garder per-port pour ne pas double-compter.
            "xdp_hw_per_port":         (data.get("xdp") or {}).get("hw_max_combined"),
            ***REMOVED*** ── Socle DPDK narrow (bloc `rl` du moteur ≥0.39.16) : sous PF vfio le budget pertinent
            ***REMOVED*** n'est plus les files AF-XDP (hw_max_combined=None, pas de netdev kernel) mais les
            ***REMOVED*** SESSIONS RL par port en TX (cap RL_TX_QUEUES_CAP — docs/chantiers/DPDK_NARROW.md §7) et les files RSS
            ***REMOVED*** en RX. rl_tx_dropped = sessions TX au-delà du cap, IGNORÉES par le moteur → badge
            ***REMOVED*** SUR-CAPACITÉ côté UI. Cap par port : live du moteur, sinon repli orchestrateur
            ***REMOVED*** (profil mesuré / bibliothèque de cartes) quand le moteur est down ou pré-0.39.16.
...
def _compute_senders_detail(only_vmid=None):
    ...
    for s in send_snapshot:
        ***REMOVED*** tag vmid : peut être absent ou « None » (sender mal enregistré) → on ignore ce sender
        ***REMOVED*** au lieu de laisser int() lever (ValueError) et faire 500 sur TOUTE la page I/O.
    ...
    for vmid, snds in sorted(by_container.items()):
        c = by_vmid.get(vmid, {})
        ip = c.get("ip") or get_container_ip(vmid)
        ***REMOVED*** deploy_config pour récupérer les shm_name côté input des senders
        dc = c.get("deploy_config")
        try:
            dc = _json.loads(dc) if isinstance(dc, str) else dc
        except Exception:
        ...
        ***REMOVED*** Multi-NIC : ports média du nœud + assignation effective par slot TX (auto modulo / épinglé).
        from .. import docker_driver as _dd
        from ..database import db_get_node as _dgn
        _node = _dgn(c.get("node_id")) if c.get("node_id") else None
        ...
        for s in sorted(snds, key=_sort_key):
            ...
            ***REMOVED*** Format réel du slot TX : dc_params.tx_slots[tx_idx] porte les vraies dimensions (posées
            ***REMOVED*** au déploiement), alors que dc_params.video est un défaut global (souvent 1280x720). On
            ***REMOVED*** lit donc le slot d'abord, repli sur video puis défaut.
            _txsl = {}
            ...
            senders_out.append({
                ...
                "fps": live.get("fps"),
                ***REMOVED*** Santé TX (moteur 2110_io) : cadence nominale + trames ayant raté leur epoch
                ***REMOVED*** (sous-cadence = scheduler saturé → « RTP alignment failure » au récepteur).
                "fps_nominal": live.get("fps_nominal"),
                "late": live.get("late"),
                ***REMOVED*** ⚠ `fps` N'EST PAS LA CADENCE DU FIL — ce commentaire l'affirmait, et c'est faux.
                ***REMOVED*** `mtl_rx.c:write_stats` le dit explicitement : le rejeu n'est PAS comptabilisé
                ***REMOVED*** (libmtl n'offre aucun signal fiable pour le compter), donc `fps` publie « les
                ***REMOVED*** trames NEUVES produites par le worker » et « SOUS-ESTIME la cadence du fil quand
                ***REMOVED*** la source est déficitaire ». La cadence réelle est garantie nominale par
                ***REMOVED*** l'horloge de sortie et ne se vérifie qu'au fil (compteurs de port, ou récepteur
                ***REMOVED*** tiers — cf. [[neuron-json-witness-endpoint]]).
                ***REMOVED*** Vécu le 2026-08-09 : `fps` à 37,8 sur un TX parfaitement alimenté ; le Neuron
                ***REMOVED*** verrouillait un 1080p50 propre avec ZÉRO erreur de séquence RTP au même instant.
                ***REMOVED*** Sur ce chemin, seuls `repeats` et `late` (compteurs d'ÉVÉNEMENTS) sont probants.
                ***REMOVED*** `fps_source` = fps − rejeu/s ; `repeats` = cumul. Absents (vieille image
                ***REMOVED*** moteur) → None, pas d'exception.
                "fps_source": live.get("fps_source"),
                "repeats": live.get("repeats"),
                ***REMOVED*** Famine TX : activé/câblé mais n'émet aucun flux (cf. metrics.tx_stalled_cache).
                "tx_stalled": (bool(_txst.get(vmid, {}).get(st.get("tx_idx"))) if essence == "video" else None),
                ***REMOVED*** Format vidéo (résolution + scan + chroma + profondeur) : slot TX réel > video > défaut.
                ...
                ***REMOVED*** Port (NIC) effectif du slot TX (multi-NIC) : {iface, pinned} ou None si mono-port.
...
def api_io_mtl():
    ...
    for c in db_get_containers():
        ...
        ***REMOVED*** Collecter tous les receivers (vidéo + audio + ANC) pour les associer par slot.
        ...
        for r in rx_video:
            ...
            rx.append({"slot": v_idx, "active": bool(r.get("active")),
                       ...
                       "anc":   [{"slot": a.get("idx"), "active": bool(a.get("active")),
                                 ...
                                 for a in ancs],
                       "leg2": None})
        ***REMOVED*** fps live des senders TX, indexés par id NMOS
        live_by_id = {}
        for blk in _compute_senders_detail(only_vmid=vmid):
            for s in blk.get("senders", []):
                live_by_id[s["id"]] = s
        def _ess(t, pfx):   ***REMOVED*** destination d'une essence : leg0 + leg1 (2022-7)
        ...
        ***REMOVED*** PRÉSERVER un 0 explicite (aucun TX actif) : `or 6` traiterait 0 comme « non défini » → 6
        ***REMOVED*** senders fantômes. On ne retombe sur 6 que si la clé est absente/None.
        ...
        ***REMOVED*** « Option A » : audios/ANC par slot pilotés par tx_flows (attached_to), N quelconque. Map
        ***REMOVED*** (essence, idx)→flow_id pour l'add/remove granulaire côté UI Destinations.
        from .. import io2110_flows as _iof
        _tx_flows = _iof.active_flows(params, "tx")
        _tx_id_of = {(f["essence"], f["idx"]): f["id"] for f in _tx_flows}
        ***REMOVED*** C2b+ : ressource NMOS bindée par slot/essence (rebinding explicite) — {id,label} ou None.
        ...
        ***REMOVED*** Port (NIC) effectif par slot TX (multi-NIC) : auto modulo / épinglé via tx_pins.
        ...
        for i, t in enumerate(tx_slots_full[:active_tx_count]):
            ...
            ***REMOVED*** SDP PAR ESSENCE. NMOS publie un sender distinct par essence (vidéo, chaque audio,
            ***REMOVED*** ANC), chacun avec son propre fichier de transport. La page ne servait qu'un seul
            ***REMOVED*** « SDP » par slot, construit sur le sender VIDÉO : ouvert sur un slot vidéo+audio, il
            ***REMOVED*** montrait un SDP sans audio dedans — de quoi conclure que l'audio n'est pas annoncé
            ***REMOVED*** alors qu'il l'est, sous un autre sender. Chaque essence porte donc désormais le sien.
            def _sdp(sid_):
                return ("/x-nmos/connection/{}/single/senders/{}/transportfile"
                        .format(_nmos.IS05_VERSION, sid_)) if sid_ else None
            ***REMOVED*** Format AUDIO d'une sortie 2110-30. Ce n'est PAS une mesure — c'est ce que le moteur
            ***REMOVED*** crée, en dur, dans mtl_rx.c : `ops.fmt = ST30_FMT_PCM24` (L24) et
            ***REMOVED*** `ops.sampling = ST30_SAMPLING_48K`. Le nombre de canaux vient de sa config avec 8 en
            ***REMOVED*** défaut (`jint(j,"channels",8)`) et l'orchestrateur ne l'envoie jamais → 8 aujourd'hui.
            ***REMOVED*** On le calcule ICI, à un seul endroit, plutôt que d'écrire « 48kHz / L24 / 8ch » dans
            ***REMOVED*** l'interface : le jour où le nombre de canaux devient réglable, la page suivra au lieu
            ***REMOVED*** de mentir. (Le ptime, lui, est déjà PAR SORTIE — il voyage avec chaque flux.)
            _a_fmt = {"sample_rate": 48000, "bit_depth": 24,
                      "channels": int(params.get("tx_audio_channels") or 8)}
            ***REMOVED*** Chemin MXL D'ENTRÉE par essence : c'est le pendant exact du chemin de sortie affiché
            ***REMOVED*** par source en réception. La vidéo l'exposait déjà (shm_in) ; les audios et l'ANC ont
            ***REMOVED*** le leur depuis toujours (`tx_audio{i}_shm` / `tx_anc{i}_shm`, câblés par
            ***REMOVED*** docker_driver) sans qu'aucune page ne le montre — donc impossible de vérifier d'un
            ***REMOVED*** coup d'œil qu'une sortie audio lit bien le bus qu'on croit.
            ...
            def _aud(ai, _t=t, _aidxs=_aud_idxs, _i=i):
                ...
                return {"mcast": a.get("multicast_ip"), "port": a.get("dest_port"),
                        ...
                        "tone": a.get("tone"),   ***REMOVED*** config générateur de tonalité (UI éditeur TONE)
                        "sdp_href": _sdp(_sida),
                        "shm_in": (params.get(cle_tx_audio_shm(_aidx)) or "") if _aidx is not None else "",
                        ***REMOVED*** ptime PAR SORTIE (ms) : la seule part du format audio qui varie
                        ***REMOVED*** réellement d'un flux à l'autre.
                        "ptime": a.get("ptime"),
            ...
            tx.append({"slot": i,
                       ...
                       ***REMOVED*** Santé TX : sous-cadence (fps < nominal) ou late > 0 = epochs ratés
                       ***REMOVED*** (scheduler saturé) → badge ⚠ côté UI Destinations.
                       "fps_nominal": (live_by_id.get(sid) or {}).get("fps_nominal"),
                       "late": (live_by_id.get(sid) or {}).get("late"),
                       ***REMOVED*** ⚠ `fps` n'est PAS la cadence du fil (cf. le bloc détaillé plus haut dans
                       ***REMOVED*** ce fichier) : c'est le nombre de trames NEUVES prises par le worker, et il
                       ***REMOVED*** sous-estime le fil quand la source est déficitaire. Le fil, lui, est tenu
                       ***REMOVED*** par l'horloge de sortie. `fps_source` = fps − rejeu/s, `repeats` = cumul.
                       ...
                       "sender_id": sid,
                       ***REMOVED*** flow_id (« Option A ») : vidéo du slot + ANC, pour l'add/remove granulaire UI.
                       "flow_id": _tx_id_of.get(("video", i)),
                       "anc_flow_id": _tx_id_of.get(("anc", i)),
                       ***REMOVED*** Conservé pour compatibilité : c'est le SDP de la VIDÉO du slot. Les
                       ***REMOVED*** consommateurs doivent préférer `video.sdp_href` / `audios[].sdp_href` /
                       ***REMOVED*** `anc.sdp_href`, qui disent de quelle essence ils parlent.
                       "sdp_href": _sdp(sid),
                       ...
                       ***REMOVED*** >0 = grille d'émission décalée de N µs (TROFF au SDP) → sélecteur UI.
                       "epoch_shift_us": int(t.get("epoch_shift_us") or 0),
                       ***REMOVED*** Choix de la trame émise (mode tranche) : 1 = la plus RÉCEMMENT prête
                       ***REMOVED*** (~1 image de latence en moins), 0 = la plus ancienne (historique).
                       ***REMOVED*** ⚠ Le défaut ne se REDÉRIVE PAS ici : on LIT celui de `docker_driver`, qui
                       ***REMOVED*** est ce que le moteur reçoit réellement. Les deux valeurs étaient calculées
                       ***REMOVED*** séparément avec la même règle écrite deux fois — donc deux occasions de
                       ***REMOVED*** diverger, et un affichage qui aurait menti sur l'état réel de la sortie.
                       "serve_newest": (_TX_SN_DEFAUT if t.get("serve_newest") is None
                                        else (1 if t.get("serve_newest") else 0)),
                       ***REMOVED*** Format GÉN configuré du slot (repli sur le format moteur) → présélection UI.
                       ***REMOVED*** fps_cfg ≠ fps (live) ci-dessus.
                       ...
                       ***REMOVED*** PROFONDEUR EFFECTIVE de la sortie : celle du SLOT si elle y est posée,
                       ***REMOVED*** sinon celle du moteur. On lisait le moteur seul — donc une sortie réglée
                       ***REMOVED*** en 10 bits s'affichait avec les 8 bits du moteur, et le réglage qu'on
                       ***REMOVED*** venait d'appliquer semblait n'avoir rien fait.
                       ***REMOVED*** Le CHROMA reste un réglage moteur (l'endpoint de format ne le prend pas).
                       ***REMOVED*** La COLORIMÉTRIE n'est volontairement pas remontée : le moteur ne la
                       ***REMOVED*** connaît pas (script.py n'a que chroma/bit_depth), et afficher un « 709 »
                       ***REMOVED*** que rien ne pose ferait passer une supposition pour une déclaration.
                       "chroma":    params.get("chroma"),
                       "bit_depth": t.get("bit_depth") or params.get("bit_depth"),
                       ***REMOVED*** Port (NIC) effectif du slot TX (multi-NIC) : {iface, pinned} ou None.
                       "port":    _slot_tx(i)})
        ***REMOVED*** Stats NIC pour l'onglet Destinations (TX)
        _rx_blk = next((_b for _b in _compute_receivers_detail(only_vmid=vmid)), {})
        ***REMOVED*** Ventilation par port physique : on réutilise les descripteurs moteur calculés côté RX
        ***REMOVED*** (débit mesuré / files / lien, role-agnostique) et on y ajoute le compte de flux TX par
        ***REMOVED*** port (le rx_flow_count vient déjà du bloc RX). Mono-port → [] (UI = tuile agrégée).
        ...
        ***REMOVED*** Plafond TX activable = budget de queues XDP partagé (live si le contrôleur l'expose,
        ***REMOVED*** sinon réglage). On reporte ce plafond comme `tx_count` → le bouton « + Ajouter un TX »
        ***REMOVED*** n'apparaît que tant qu'il reste du budget de queues (et ne pousse jamais vers l'ENOMEM).
        _live_total = _rx_blk.get("xdp_hw_per_port")   ***REMOVED*** PAR PORT (≠ agrégat affiché) — × nb de ports ci-dessous
        ***REMOVED*** Budget AGRÉGÉ : chaque port média a son propre budget de files → × nb de ports (round-robin).
        ...
        ***REMOVED*** Socle DPDK narrow : sous pacing RL le plafond TX = budget de sessions RL (cap RL/port ×
        ***REMOVED*** ports — la limite dure constatée au banc, docs/chantiers/DPDK_NARROW.md §7), PAS les files AF-XDP (le
        ***REMOVED*** repli réglage 48 sur-plafonnait ou sous-plafonnait selon la carte). None sur un nœud
        ***REMOVED*** af_xdp → budget partagé historique inchangé.
        _cap_rx, _cap_tx = _mtl_active_caps(params, _total_q, node=node,
                                            tx_budget=_mtl_rl_tx_budget(_rx_blk, params, node))
        ***REMOVED*** Configuration enregistrée mais PAS appliquée : un moteur ne relit son adressage réseau
        ***REMOVED*** qu'au déploiement. Sans ce champ, un opérateur qui change une interface la voit
        ***REMOVED*** confirmée à l'écran et n'a aucun moyen de savoir que le moteur tourne encore sur
        ***REMOVED*** l'ancienne (cf. docker_driver.derive_config_moteur).
        try:
            from ..docker_driver import derive_config_moteur as _derive
            _ecarts = _derive(vmid)
        except Exception:
            _ecarts = []
        engines.append({"vmid": vmid, "hostname": c.get("hostname") or "***REMOVED***{}".format(vmid),
                        "node": node.get("name") or node.get("host") or "",
                        "config_non_appliquee": _ecarts,
                        ...
                        "rx": rx, "tx": tx,
                        ***REMOVED*** Ports média du nœud + stats ventilées par port (multi-NIC) : [] si mono-port.
                        ...
                        ***REMOVED*** Queues AF_XDP (sessions actives / budget) — passthrough pour le compteur
                        ***REMOVED*** ⬡ du header Destinations (io2110.js). Null tant que l'image n'expose pas xdp.
                        ...
                        ***REMOVED*** Supervision RL (socle DPDK narrow) — passthrough du bloc RX (cf.
                        ***REMOVED*** _compute_receivers_detail) : l'UI Destinations remplace la barre
                        ***REMOVED*** « Queues XDP » par « Sessions TX (RL) » quand rl_active est vrai.
