...
# Types que `coerce_config` sait traiter. TOUT LE RESTE tombe dans sa branche « texte » et se
# fait convertir en CHAÎNE, en silence.
...
# ── Capacité matérielle DECKLINK ────────────────────────────────────────────────────────────
# Même patron que `needs_dpdk` : le manifeste déclare un NOM DE CAPACITÉ, jamais un chemin.
#
# ★ C'est ce qui rend le mécanisme sûr. La page Catalogue installe des plugins depuis GitHub :
#   si un manifeste pouvait déclarer les périphériques qu'il veut (`host_devices: ["/dev/*"]`),
#   un plugin tiers obtiendrait l'accès aux disques du nœud sans que personne ne l'ait relu.
#   Avec un nom de capacité, il ne peut demander qu'une chose CONNUE, dont l'orchestrateur seul
#   décide ce qu'elle recouvre — ici les nœuds `/dev/blackmagic/*` et la bibliothèque du pilote.
