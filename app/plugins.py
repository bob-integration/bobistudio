...
A *plugin* is a self-contained, versioned container type living in `plugins/<type>/`:
    ...
    plugins/<type>/
      plugin.json     ***REMOVED*** manifest (see REQUIRED_KEYS below)
      script.py       ***REMOVED*** str.format template -> deployed to the container as /opt/script/main.py
      hooks.py        ***REMOVED*** optional — lifecycle hooks executed in-process (before_deploy, …)
      control.html    ***REMOVED*** UI fragment injected into the plugin shell
      control.js      ***REMOVED*** registers window.MXLPlugins["<type>"] = { mount(el, vmid) }
      control.css     ***REMOVED*** optional
...
def rate_nd(v):
    ...
    inline des plugins de grille — sert au format injecté au câblage (UDC) et au gating ***REMOVED***27."""
    try:
...
***REMOVED*** type -> manifest dict (augmented with "_dir" = absolute plugin directory)
REGISTRY = {}

***REMOVED*** nom_dossier -> raison (plugins présents sur disque mais NON chargés au dernier scan).
***REMOVED*** Rempli par _scan(), exposé via scan_errors() pour l'onglet de gestion des plugins.
SCAN_ERRORS = {}
...
def _scan():
    ...
    for name in sorted(os.listdir(PLUGINS_DIR)):
        ...
        if missing:
            ...
            continue
        manifest["_dir"] = pdir
        ***REMOVED*** str.format dry-run guard: catches a literal brace that wasn't doubled before
        ***REMOVED*** it ever reaches a real deployment.
        tpl_path = os.path.join(pdir, manifest["script_template"])
        try:
            with open(tpl_path, "r", encoding="utf-8") as f:
                tpl = f.read()
            rendu = tpl.format(config="{}", hostname="x", plugin_version="0")
            ***REMOVED*** ⚠ ET LE SCRIPT RENDU DOIT COMPILER. Le dry-run `.format` ci-dessus ne regarde que
            ***REMOVED*** les accolades ; pyflakes, lui, analyse le TEMPLATE, où `{config}` se lit comme un
            ***REMOVED*** littéral d'ensemble parfaitement valide. Ni l'un ni l'autre n'attrape une faute de
            ***REMOVED*** syntaxe Python — et une seule suffit à mettre le conteneur en boucle de
            ***REMOVED*** redémarrage, sans que rien ici ne l'ait signalé.
            ***REMOVED*** Vu le 2026-08-26 : un `global X` placé après une lecture de `X` dans la même
            ***REMOVED*** fonction (SyntaxError). Le plugin passait le scan, se déployait, et le conteneur
            ***REMOVED*** relançait le script toutes les deux secondes.
            ...
            ***REMOVED*** Une accolade non doublée sous forme `{config[x]}` lève TypeError et
            ***REMOVED*** `{config.attr}` lève AttributeError : les attraper aussi, sinon l'exception
            ***REMOVED*** remonte jusqu'à REGISTRY = _scan() (niveau module) → ImportError au boot.
            ...
            continue
        _verifier_types_config(manifest)
...
***REMOVED*** Types que `coerce_config` sait traiter. TOUT LE RESTE tombe dans sa branche « texte » et se
***REMOVED*** fait convertir en CHAÎNE, en silence.
TYPES_CONFIG = {"number", "checkbox", "bool", "multiselect", "select", "text", "textarea"}


def _verifier_types_config(manifest):
    """Signale un `type` de `config_schema` que `coerce_config` ne connaît pas.

    ⚠ CE GARDE-FOU EXISTE PARCE QUE LE DÉFAUT EST ARRIVÉ. Un champ déclaré `"type": "list"`
    (2026-09-07, plugin `scope`) n'est traité par aucune branche de `coerce_config` : il finit
    dans le `else` final, qui fait `str(v)`. Une LISTE devenait donc la CHAÎNE de son propre
    repr — `"['waveform', 'vecteur']"` — persistée telle quelle, et `derive_wiring` itérait
    dessus CARACTÈRE PAR CARACTÈRE en déclarant une sortie nommée `_[`, une `_'`, une `_w`…
    Rien ne plantait, rien n'était logué, et le plugin fonctionnait par ailleurs.

    On ne refuse pas le plugin : un type inconnu ne casse pas le rendu, et écarter un plugin
    entier pour ça serait pire que le défaut. On le DIT, une fois, au démarrage."""
    for f in (manifest.get("config_schema") or []):
        t = f.get("type")
        if t and t not in TYPES_CONFIG:
            log.warning("plugins: %s — champ `%s` de type inconnu « %s » : coerce_config le "
                        "convertira en CHAÎNE. Types connus : %s",
                        manifest.get("type"), f.get("key"), t, ", ".join(sorted(TYPES_CONFIG)))
...
***REMOVED*** ── Fraîcheur du registre (bug 2026-07-14) ───────────────────────────────────────────────
***REMOVED*** REGISTRY était scanné UNE FOIS au boot. Après un bump de plugin SANS redémarrage :
***REMOVED***   • render_script() lit script.py sur le DISQUE      → le conteneur reçoit le code NEUF ;
***REMOVED***   • resolved_version() lit le REGISTRE EN MÉMOIRE    → deploy_config est estampillé à
***REMOVED***     l'ANCIENNE version.
***REMOVED*** Résultat : des conteneurs qui exécutent le bon code avec une mauvaise étiquette — et comme la
***REMOVED*** garde d'application à chaud compare cette étiquette à la version du manifeste (deploy.py), elle
***REMOVED*** échoue TOUJOURS : chaque réglage d'un mur devenait un REDÉPLOIEMENT COMPLET (blip) au lieu d'un
***REMOVED*** push à chaud. Le registre se rafraîchit donc quand un manifeste change sur le disque.
_FINGERPRINT = {}          ***REMOVED*** {chemin plugin.json: mtime} au dernier scan
_FP_LAST_CHECK = [0.0]     ***REMOVED*** anti-martèlement : au plus un stat de la liste par seconde
...
***REMOVED*** ── Capacité matérielle DECKLINK ────────────────────────────────────────────────────────────
***REMOVED*** Même patron que `needs_dpdk` : le manifeste déclare un NOM DE CAPACITÉ, jamais un chemin.
***REMOVED***
***REMOVED*** ★ C'est ce qui rend le mécanisme sûr. La page Catalogue installe des plugins depuis GitHub :
***REMOVED***   si un manifeste pouvait déclarer les périphériques qu'il veut (`host_devices: ["/dev/*"]`),
***REMOVED***   un plugin tiers obtiendrait l'accès aux disques du nœud sans que personne ne l'ait relu.
***REMOVED***   Avec un nom de capacité, il ne peut demander qu'une chose CONNUE, dont l'orchestrateur seul
***REMOVED***   décide ce qu'elle recouvre — ici les nœuds `/dev/blackmagic/*` et la bibliothèque du pilote.
def needs_decklink(type_):
    """Vrai si le type exige le matériel Blackmagic (manifeste `needs_decklink`). Le conteneur
    reçoit alors les nœuds `/dev/blackmagic/*` ÉNUMÉRÉS SUR LE NŒUD (ils n'existent qu'avec une
    carte) et un bind lecture seule de `libDeckLinkAPI.so` — le pilote s'installant à la main,
    rien de Blackmagic n'est embarqué dans nos images."""
    m = REGISTRY.get(type_) or {}
    return bool(m.get("needs_decklink"))
...
def sections():
    ...
    for m in REGISTRY.values():
        if m.get("type") in dis:        ***REMOVED*** désactivé → hors palette/nav
            continue
        nav = m.get("nav") or {}
        sec = nav.get("section")
        if not sec:
            continue
        ***REMOVED*** Route de la RUBRIQUE = toujours le shell unifié /{sec} (page à onglets). On n'hérite
        ***REMOVED*** PAS du nav.route d'un plugin : celui-ci est un favori legacy par-type (ex.
        ***REMOVED*** /traitements/correcteurs → redirige en /traitements***REMOVED***color_corrector) et, selon l'ordre
        ***REMOVED*** de scan, il forçait le lien de nav sur un onglet précis via un ***REMOVED***hash (cf. Correcteur
        ***REMOVED*** toujours sélectionné). Les routes par-plugin restent dispo dans chaque manifeste (nav.route).
        entry = out.setdefault(sec, {
            ***REMOVED*** Libellé de RUBRIQUE (« Traitements », « Médias ») : même convention de clé que les
            ***REMOVED*** types, côté cœur — cf. libelle_type().
            "label": _traduit(f"section.nav.{sec}", nav.get("label", sec.capitalize())),
            "route": f"/{sec}",
            "order": nav.get("order", 100),
            "plugins": [],
        })
        ***REMOVED*** Copie SUPERFICIELLE : REGISTRY est partagé (scanné une fois) et sert aussi au rendu de
        ***REMOVED*** script ; y écrire un label traduit contaminerait tout le process, langue comprise.
        entry["plugins"].append(dict(m, label=libelle_type(m.get("type"))))
        ***REMOVED*** section-level order = smallest plugin order
...
def versions(type_):
    ...
    if os.path.isdir(vdir):
        ...
        ***REMOVED*** tri décroissant « façon version » (numérique par segment, fallback lexical)
        def _key(v):
            try:
...
***REMOVED*** Métadonnées par version (lecture seule côté UI) : changelog + dates.
***REMOVED*** Stockées dans un meta.json optionnel à côté du script.py de chaque version
***REMOVED*** (plat = courante, sinon versions/<ver>/meta.json). Aucun plugin existant n'en a
***REMOVED*** aujourd'hui → lecture tolérante avec défauts vides (rétrocompat).
...
***REMOVED*** Un titre de section de changelog porte souvent sa version en fin : « … (0.66.0) ».
_META_VER_RE = re.compile(r"\((\d+(?:\.\d+)+)\)\s*$")
***REMOVED*** Classement par PRÉFIXE du titre. Tout le reste est une nouveauté : le parc titre ses
***REMOVED*** sections librement (« Libellé de la source sous chaque Rx … »), et une section non
***REMOVED*** reconnue doit s'afficher, pas disparaître.
_META_KIND_PREFIX = (
...
def parse_meta_changes(raw_changes, version=None):
    ...
    else:                                   ***REMOVED*** format plat (legacy) : une simple liste
...
def read_version_meta(type_, version=None):
    ...
    if p and os.path.isfile(p):
        ...
        if isinstance(raw, dict):
            ...
            ***REMOVED*** Format plat (legacy) : fixes/known_bugs vivent à la racine du meta.json.
...
def render_script(type_, params, hostname, version=None):
    ...
    with open(path, "r", encoding="utf-8") as f:
        tpl = f.read()
    cfg = dict(params or {})
    ***REMOVED*** Filet : un multiview sans format de sortie explicite hérite du défaut SYSTÈME (jamais un
    ***REMOVED*** littéral) — couvre les conteneurs legacy/edge non semés à la création. L'explicite prime.
    if type_ == "multiview":
        from .scripts import multiview_output_format_defaults
        for k, v in multiview_output_format_defaults().items():
            if not cfg.get(k):
                cfg[k] = v
    ***REMOVED*** Même filet pour TOUT TYPE QUI DÉCLARE UN CANVAS : format sans valeur explicite → format
    ***REMOVED*** par défaut SYSTÈME (jamais le littéral 720p du plugin.json — cf. effective_deploy_defaults).
    ***REMOVED***
    ***REMOVED*** ⚠ C'ÉTAIT UNE LISTE EN DUR ("mixer", "avsync"), et elle a récidivé : la sonde de latence,
    ***REMOVED*** ajoutée le 2026-08-11, a publié son panneau en 1280×720p25 8 bits sur une maison en
    ***REMOVED*** 1920×1080p50 10 bits — parce que personne n'a pensé à l'inscrire ici. C'est exactement la
    ***REMOVED*** panne que `_FORMAT_NOT_FROM_SETTINGS` documente deux fonctions plus bas, reproduite par le
    ***REMOVED*** filet censé l'empêcher. La règle est donc la MÊME des deux côtés, et déclarative : un type
    ***REMOVED*** qui déclare width+height suit le format système, sauf exclusion explicite et justifiée.
...
def options_dynamiques(source):
    ...
    if source == "tally_levels":
        ***REMOVED*** Les niveaux de tally sont des ENTITÉS NOMMÉES du site, allouées dans Réglages →
        ***REMOVED*** Labels & Tally. Un plugin ne peut ni les deviner ni en figer la liste : elle change
        ***REMOVED*** dès qu'une production est créée. Le libellé montre le nom écrit par l'exploitant —
        ***REMOVED*** « niveau 7 » ne dit rien, « 7 — Antenne » se choisit sans aller vérifier ailleurs.
        ***REMOVED*** ★ LA VALEUR EST L'UUID, LE LIBELLÉ PORTE LE NUMÉRO. Le numéro n'est qu'un rang
        ***REMOVED*** d'affichage : réordonner les niveaux le réécrit, et une configuration qui l'aurait
        ***REMOVED*** mémorisé pointerait ensuite un autre niveau — silencieusement, sur une fonction
        ***REMOVED*** d'antenne. L'exploitant lit « 3 — Plateau », le manifeste garde l'identité.
        try:
    ...
    try:
        ...
        return []
    ***REMOVED*** ⚠ CE LIBELLÉ EST DU TEXTE D'INTERFACE, pas une donnée. Les NOMS de préréglages
    ***REMOVED*** viennent des réglages du site et ne se traduisent pas — celui-ci, si. Écrit en
    ***REMOVED*** dur, il restait français dans une interface en anglais (constaté en recette).
    ...
    for l in lignes:
        ...
        if not nom:
            continue
        ***REMOVED*** Le libellé montre ce que le nom cache : « HD 1080i50 » ne dit ni la
        ***REMOVED*** chroma ni la profondeur, et c'est justement ce qu'on ne veut pas
        ***REMOVED*** laisser deviner.
        détail = ""
        if len(parts) >= 4:
...
def config_schemas():
    ...
    for t, m in REGISTRY.items():
        ...
        for f in cs:
            ...
            ***REMOVED*** Options dynamiques : résolues À LA LECTURE, jamais écrites dans le
            ***REMOVED*** registre (qui est partagé — y figer une liste la périmerait pour
            ***REMOVED*** tout le process au premier changement de réglage).
...
def _champ_traduit(type_, champ):
    ...
    f = dict(champ)
    ***REMOVED*** Un manifeste peut DÉCLARER lui-même sa clé (`label_i18n`/`help_i18n`) : cette convention
    ***REMOVED*** existait avant la nôtre, ses clés sont au catalogue du cœur — mais rien ne les lisait, donc
    ***REMOVED*** le champ restait français alors que sa traduction était écrite (constaté 2026-08-25).
    ***REMOVED*** La déclaration explicite gagne ; sinon on retombe sur la convention `type.<id>.cfg.<clé>.*`.
    ***REMOVED*** ⚠ Un champ peut n'avoir QUE `label_i18n`, sans `label` : le libellé est alors absent du
    ***REMOVED*** manifeste et l'UI retombe sur la CLÉ TECHNIQUE (« light_angle » à l'écran). Il faut donc
    ***REMOVED*** résoudre la déclaration même quand `label` est vide — c'était le cas des quatre champs
    ***REMOVED*** d'éclairage et de transition de `split` (constaté 2026-08-25).
...
def manifest_summary_for_js():
    ...
    for t, m in REGISTRY.items():
        ...
        entry = {
            ...
            ***REMOVED*** Le badge porte souvent une forme COURTE distincte du label (« SONDE », « AV Sync ») :
            ***REMOVED*** il a donc sa propre clé, avec repli sur le badge du manifeste puis sur le label.
        ...
        ***REMOVED*** aggregate_fps=false → un fps de carte agrégé n'a pas de sens (moteur multi-flux) :
        ***REMOVED*** la page Câbles masque le badge fps de carte et affiche le fps par port à la place.
...
***REMOVED*** Types qui déclarent un canvas (width/height) mais dont le format N'EST PAS celui de la maison :
***REMOVED*** il est imposé par la SOURCE. `stream_in` ingère un flux externe — lui coller le format système
***REMOVED*** serait un mensonge, pas un défaut utile.
...
def effective_deploy_defaults(type_):
    ...
    if _canvas:
        try:
            ...
            ***REMOVED*** chroma/profondeur suivent la MÊME règle que la géométrie : un plugin qui déclare
            ***REMOVED*** ces clés les tient des Réglages, sinon on aurait corrigé la moitié du format.
            if "scan" in dd:
            ...
            pass
    ***REMOVED*** Mode tranche GLOBAL : si le setting slice_mode_global est actif ET que le config_schema du
    ***REMOVED*** type porte slice_mode → défaut effectif slice_mode=True (+ cadence "flow" UNIQUEMENT pour
    ***REMOVED*** les types dont le schema porte cadence). L'explicite (POST/params persistés) PRIME toujours
    ***REMOVED*** (merge défauts ← existants ← POST aux 3 points d'entrée : création, /deploy, /plugin_config).
    ***REMOVED***
    ***REMOVED*** ★ `gpu_slice` EST POSÉ AUSSI depuis le 2026-08-11, et c'est un correctif, pas un élargissement.
    ***REMOVED*** Il en était exclu (« décision séparée, banc gate GPU ») à une époque où le GPU n'avait pas de
    ***REMOVED*** verdict. Conséquence : sur un nœud GPU, `SLICE_ON` exige les DEUX — activer le réglage global
    ***REMOVED*** laissait donc tout mur GPU SILENCIEUSEMENT en image entière. On croyait avoir tranché la
    ***REMOVED*** chaîne, un maillon ne l'était pas, et le bénéfice disparaissait partout sans aucun message :
    ***REMOVED*** exactement le scénario de la pyramide monolithique, reproduit par le réglage censé l'empêcher.
    ***REMOVED*** Verdict obtenu depuis, MESURÉ EN PRODUCTION (mur 906, Quadro P5000, 1920x1080p50) : 50,0 fps
    ***REMOVED*** tenus, zéro trame perdue, 13 ms de coût propre pour 20 de budget. Un mur qui n'en voudrait pas
    ***REMOVED*** se pose `gpu_slice: false` explicitement — l'explicite prime, c'est tout l'intérêt du merge.
    try:
...
def validate_config(type_, params):
    ...
    for f in schema:
        ...
        ***REMOVED*** RÉGLAGE NULLABLE : un champ dont le manifeste déclare `"default": null` a un état
        ***REMOVED*** « non réglé » qui est une VALEUR, pas une valeur manquante — « ligne mesurée » vide
        ***REMOVED*** veut dire « toutes les lignes ». Sans cette exception, ce champ pouvait être RÉGLÉ
        ***REMOVED*** mais jamais REMIS À VIDE : le POST était refusé « n'est pas un nombre », et un
        ***REMOVED*** conteneur restait bloqué sur la ligne 0 à chaque redéploiement — un waveform d'une
        ***REMOVED*** seule ligne, qui ressemble à un waveform en panne.
        ...
        if t == "number":
            ...
            ***REMOVED*** ★ UNE LISTE, ET LE CAS « UN SEUL » N'EN EST QUE LE PREMIER ÉLÉMENT. Le tally se
            ***REMOVED*** CUMULE : une même source peut être suivie sur plusieurs chaînes de destination, et
            ***REMOVED*** un champ scalaire obligeait à choisir laquelle compte. On tolère un scalaire en
            ***REMOVED*** entrée (schémas hérités) plutôt que de refuser un réglage déjà en base.
            ...
            ***REMOVED*** Même résolution que côté affichage : sans elle, un select à options
            ***REMOVED*** dynamiques n'aurait AUCUN garde-fou — le champ le plus permissif du
            ***REMOVED*** schéma serait celui dont la liste est la mieux définie.
            _opts = f.get("options") or (options_dynamiques(f["options_from"])
                                         if f.get("options_from") else [])
...
***REMOVED*** Écrêtages déjà signalés (type, clé, valeur brute) — une alerte par valeur fautive, pas par
***REMOVED*** appel (coerce_config est appelé à chaque déploiement / rendu de script).
...
def coerce_config(type_, params):
    ...
    for f in schema:
        ...
        t = f.get("type")
        v = out[k]
        ***REMOVED*** Pendant du test de `validate_config` : un réglage déclaré nullable revient à
        ***REMOVED*** l'ABSENCE, il n'est pas rabattu sur une borne. `float(None or 0)` valait 0, donc le
        ***REMOVED*** minimum — et « aucune ligne choisie » devenait « ligne 0 », silencieusement.
        if v in (None, "") and "default" in f and f["default"] is None:
            out.pop(k, None)
            continue
        if t == "number":
            ...
            ***REMOVED*** "bool" est un alias de "checkbox". Attention : bool("False") == True →
            ***REMOVED*** on interprète les chaînes explicitement.
            if isinstance(v, str):
                out[k] = v.strip().lower() in ("1", "true", "yes", "on")
            else:
                out[k] = bool(v)
        elif t == "multiselect":
            ***REMOVED*** Normalisation de SORTIE : toujours une liste, dédoublonnée, ordre préservé. Les
            ***REMOVED*** consommateurs (distributeur TSL, hooks tally_targets) n'ont ainsi qu'une forme à
            ***REMOVED*** lire — un scalaire hérité y devient la liste à un élément, sans resave.
            ***REMOVED***
            ***REMOVED*** ⚠ ON NE CONVERTIT PAS EN ENTIER. Les valeurs sont des identités opaques (UUID de
            ***REMOVED*** niveau) depuis le 2026-09-01 : `int()` les jetterait toutes, et le champ se
            ***REMOVED*** viderait à chaque enregistrement sans le moindre message.
...
def _port_format(essence, params):
    ...
    try:
        ...
        if essence == "video":
            ***REMOVED*** Fallback sur input_format (format de la source câblée, stocké par _apply_wire)
            ***REMOVED*** pour les plugins passthrough (delay, avsync) qui n'ont pas width/height propres.
            inf = p.get("input_format") or {}
            chroma = normalize_chroma(p.get("chroma") or v.get("chroma") or inf.get("chroma"))
            ***REMOVED*** out_width/out_height AVANT width/height : pour un multiview c'est out_width qui pilote
            ***REMOVED*** la sortie réelle (script.py : OUT_WIDTH = out_width or width). Les lire après width
            ***REMOVED*** faisait afficher width (canevas voulu) sur la page Câbles alors que le flux émis suivait
            ***REMOVED*** out_width → mismatch au câblage TX. Seul le multiview porte out_width (streamer ne produit
            ***REMOVED*** aucun shm), donc les autres types retombent sur width sans changement.
            ...
            ***REMOVED*** Multiview portrait : le flux émis est tourné 90° → dims SWAPPÉES (cf scripts.multiview_output_dims).
            ...
            if h: fmt["height"] = h
            if fps:
                ***REMOVED*** Cadence EXACTE (num/den) propagée pour le fractionnaire (29.97/59.94) ; `fps`
                ***REMOVED*** garde la valeur précise (29.97), `fps_num`/`fps_den` la forme rationnelle exacte
                ***REMOVED*** (consommée par l'UDC et le gating cadence ***REMOVED***27).
                num, den = rate_nd(fps)
                fmt["fps_num"] = num; fmt["fps_den"] = den
                fmt["fps"] = num if den == 1 else round(num / den, 3)   ***REMOVED*** 25 (int) / 29.97 (float)
            ***REMOVED*** Balayage (entrelacé/progressif) + ordre de champ : propagés au consommateur câblé pour
            ***REMOVED*** qu'une sortie 2110 ré-émette dans le bon format (ex. 1080i50 → scan=i, sinon 25p +
            ***REMOVED*** combing). Source de vérité = params.scan/field_order (jamais l'étiquette du flux MXL,
            ***REMOVED*** cf. interlace_mode forcé progressive côté grain). Best-effort, omis si inconnu.
...
def ports_topologie(type_, hostname, params=None):
    ...
    if hook:
        try:
            ...
            pass          ***REMOVED*** hook cassé → on retombe sur le wiring déclaratif, jamais sur rien
    ...
    def _shm(x):
        ***REMOVED*** `shm` peut être porté par un `state_field` : le flux câblé vit alors dans les params
        ***REMOVED*** (ex. `tx0_shm`), parce qu'il change à chaud sans redéploiement. C'est la résolution
        ***REMOVED*** que `deploy.py` fait déjà — l'oublier rendait des ports entiers invisibles.
        v = x.get("shm")
...
def derive_wiring(type_, hostname, params=None):
        ...
        for idx, it in enumerate(items or []):
            ...
            if "repeat" in it:
                ...
                for i, entry in enumerate(lst):
                    ...
                    copy["slot"] = i
                    copy["shm"] = raw
                    ***REMOVED*** Libellé EXPLICITE du manifeste (gabarit `{i1}`) prioritaire : une même liste
                    ***REMOVED*** à géométrie peut porter PLUSIEURS essences par entrée (multiview : la vidéo
                    ***REMOVED*** et son flux ANC) — sans ça les deux ports s'appelleraient « Entrée N ».
    ...
    ***REMOVED*** ── Pyramide : produces DYNAMIQUES = 1 proxy par (source câblée × niveau). Les sorties
    ***REMOVED*** ne sont pas déclarables en statique (elles dépendent de ce qui est câblé) → on les dérive
    ***REMOVED*** ici depuis input_0..input_{n-1} (+ input_{i}_fmt pour les dimensions/chroma). Le nommage
    ***REMOVED*** <source>__pL ET la formule de dimensions sont les MÊMES que plugins/pyramide/script.py.
    ***REMOVED*** ── Scope : un flux PAR INSTRUMENT publié ────────────────────────────────────────────
    ***REMOVED*** Comme la pyramide, ces sorties ne sont pas déclarables en statique : elles dépendent de ce
    ***REMOVED*** que l'exploitant a demandé. Le NOM porte l'instrument (`<hôte>_scope_waveform`) et non un
    ***REMOVED*** numéro — sur la page Câbles, « Sortie 2 » n'apprendrait rien à personne.
    if type_ == "scope":
        ***REMOVED*** ⚠ LE FORMAT DÉCLARÉ DES SORTIES EST CELUI DE LA SORTIE, pas celui de la source.
        ***REMOVED*** `_port_format` dérive des params `width`/`height`, c'est-à-dire du flux MESURÉ : sans
        ***REMOVED*** cette reprise, un multiview lirait « 1920×1080 » pour une mosaïque réellement produite
        ***REMOVED*** en 960×540, et dimensionnerait sa case sur un chiffre faux.
        def _fmt_sortie(l, h):
    ...
    if type_ == "pyramide":
        ...
        ***REMOVED*** Socle d'octaves : MÊME dérivation que plugins/pyramide/script.py (base_octaves, override
        ***REMOVED*** `levels`). full ½/¼/⅛/1/16 · half ½ · none aucun.
        ...
        for i in range(n_inputs):
            ...
            for L in levels:
                ...
                prod.append(p)
            ***REMOVED*** Tailles sur-mesure (mêmes nom/alignement que plugins/pyramide/script.py:_align_extra).
    ...
    return {
        ...
        ***REMOVED*** endpoint :8082 renvoyant l'état live (shm câblés) pour les consumes hot-wire
        "state_endpoint": w.get("state_endpoint", "/state"),
    }


***REMOVED*** ─── Import / export / activation de paquets .mxlplugin ─────────────
...
def validate_package(d):
    ...
    ***REMOVED*** ★ EXIGENCE DE VERSION DU CŒUR. Un composant peut avoir besoin d'un orchestrateur récent :
    ***REMOVED*** `services/tsl` importe `app.tally`, `multiview` attend que le cœur lui pousse ses colonnes
    ***REMOVED*** de libellé. Installé sur un cœur trop ancien, le premier casse BRUYAMMENT (ImportError) et
    ***REMOVED*** le second EN SILENCE — le mur affiche des libellés périmés sans que rien ne le dise. C'est
    ***REMOVED*** la seconde qui a motivé ce garde : une panne muette qu'aucun compteur ne montre.
    ***REMOVED***
    ***REMOVED*** ⚠ ON REFUSE ICI, AU PLUS PROFOND. Les trois voies d'installation — catalogue et les deux
    ***REMOVED*** imports manuels — passent toutes par cette fonction. Poser le contrôle sur les sites
    ***REMOVED*** d'appel aurait laissé le prochain en oubli silencieux, exactement le défaut relevé sur
    ***REMOVED*** l'épinglage de version (un seul appel sur 23 passait sa consigne).
    ...
    try:
        ...
        ***REMOVED*** `{config[x]}` → TypeError, `{config.attr}` → AttributeError : couvrir tous les
        ***REMOVED*** cas d'accolade non doublée (même élargissement que le dry-run de _scan()).
...
def activate_version(type_, version):
    ...
    if not os.path.isfile(mp):                 ***REMOVED*** archive script-only → reconstruit le manifeste
...
***REMOVED*** Initial scan at import time.
REGISTRY = _scan()
***REMOVED*** Empreinte des manifestes au scan initial → `rescan_if_changed()` ne verra un « changement » que
***REMOVED*** s'il y en a un vraiment (sans ça, le tout premier déploiement re-scannerait pour rien).
_FINGERPRINT.update(_manifests_fingerprint())
