...
***REMOVED*** Convention de numérotation (« le 0 n'existe pas ») : la règle, ses miroirs et ses pièges sont
***REMOVED*** documentés dans `app/numerotation.py`, qui fait foi. Ré-exportés ici parce que ce module est le
***REMOVED*** point d'entrée naturel du modèle de flux 2110.
from .numerotation import (numero, indice, cle_tx_shm, cle_tx_audio_shm,  ***REMOVED*** noqa: F401
...
***REMOVED*** ── Dérivation (MIGRATION / repli) : compteurs legacy → liste de flux ─────────────────────────
...
def derive_rx_flows(params):
    ...
    if aper > 0:
        n_audio = n_video * aper                                  ***REMOVED*** audio suit la vidéo (builder l.537)
    else:
        n_audio = min(int(arc if arc is not None else n_audio_full), n_audio_full)  ***REMOVED*** builder l.539
    n_anc = min(n_video, n_anc_full)                              ***REMOVED*** 1 ANC/vidéo, ≤ anc_count (builder l.541)
    flows, vids = [], []
...
def derive_tx_flows(params):
    ...
    flows, vids = [], []
    ***REMOVED*** ★ VIDÉO OPTIONNELLE : un slot marqué `video_off` (sortie audio-seule / ANC-seule, cf.
    ***REMOVED*** io2110_layouts.apply_layout) n'a PAS de flux vidéo → ses audio/ANC ne s'attachent à rien
    ***REMOVED*** (attached_to=None). Un slot normal (palette) n'a pas ce marqueur → comportement historique.
    for i in range(n_tx):
    ...
    for i in range(n_tx):
        ...
        ***REMOVED*** Slot vidéo : repli au pool legacy si `audios` absent. Slot sans vidéo : uniquement les audios
        ***REMOVED*** réellement déclarés (pas de repli — sinon un slot ANC-seul se verrait coller des audios).
        ...
        ***REMOVED*** ANC : slot vidéo → historique (1 ANC/slot) ; slot sans vidéo → seulement si une dest ANC est déclarée.
        if not voff or (slot or {}).get("anc_multicast_ip"):
            flows.append({"id": _new_id(), "essence": "anc", "idx": i, "attached_to": vids[i], "label": ""})
    return flows


***REMOVED*** ── Accès / introspection d'une liste de flux ────────────────────────────────────────────────

***REMOVED*** ─── Niveau d'attention PAR SOURCE (présence signal) ────────────────────────────────────────
***REMOVED*** Un gel n'est un incident que si la source est CENSÉE bouger : une mire, une ardoise ou un player
***REMOVED*** en pause sont fixes par construction, et alerter dessus apprend à l'exploitant à ignorer son fil.
***REMOVED*** Chaque source porte donc ses propres drapeaux. Réglage DE CONTENU par défaut permissif (décision
***REMOVED*** 2026-07-28) : gel/noir/silence n'alertent pas tant que personne ne l'a demandé, tandis que les
***REMOVED*** écarts de CONFORMITÉ (hors gamut, loudness) restent surveillés — ils ne dépendent pas de ce que
***REMOVED*** la source a décidé de montrer.
***REMOVED*** ⚠ Conséquence assumée : un vrai gel sur une caméra live ne dira rien tant que sa case n'est pas
***REMOVED*** cochée. C'est pour ça que l'UI doit montrer l'état de chaque source, pas seulement le proposer.
***REMOVED*** ⚠ PAS DE LOUDNESS ICI (retiré le 2026-07-28, décision utilisateur). Une mesure de loudness n'a
***REMOVED*** de sens que RAPPORTÉE À UN PROGRAMME : elle a un début et une fin, et la conformité R128 se juge
***REMOVED*** sur l'INTÉGRÉ de ce programme. Le moteur, lui, ne peut produire qu'un loudness MOMENTANÉ (~400 ms,
***REMOVED*** et seulement sur L/R avec un poids 1,0 — faux dès qu'on sort du stéréo). Comparer ce momentané à
***REMOVED*** une cible ±2 LU fait sortir n'importe quelle source vivante en permanence : un passage calme, une
***REMOVED*** respiration, un applaudissement. C'était une alarme condamnée à battre.
***REMOVED*** Le bon outil est un plugin DÉDIÉ, armé au début d'un programme et coupé à la fin. Le moteur
***REMOVED*** continue de publier `lufs` (mesure brute, utile à l'affichage) — c'est l'ALARME qui disparaît.
***REMOVED*** `clip` (saturation) : DÉFAUT ACTIF, comme le gamut. Ce n'est pas un réglage de contenu — une
***REMOVED*** source écrêtée est en défaut quelle que soit l'intention de l'exploitant, alors qu'un silence ou
***REMOVED*** un gel peuvent être parfaitement voulus.
***REMOVED*** `tx_late` (2026-07-28) : trames TX en retard — le producteur câblé sur ce SLOT DE SORTIE n'a pas
***REMOVED*** fourni l'image à temps (delta du compteur cumulatif `late` du moteur, cf. metrics.py). DÉFAUT
***REMOVED*** ACTIF, comme clip/gamut : une trame non fournie à temps est un défaut de la chaîne quelle que
***REMOVED*** soit l'intention de l'exploitant — il n'y a pas de « retard voulu » comme il peut y avoir un
***REMOVED*** « gel voulu » (mire, ardoise).
...
***REMOVED*** QUEL DRAPEAU S'APPLIQUE À QUEL RÔLE. `tx_late` n'a de sens QUE côté TX : c'est une mesure du
***REMOVED*** SLOT DE SORTIE (le producteur qui alimente ce slot a-t-il livré à temps ?), rien de comparable
***REMOVED*** n'existe côté RX. Sans cette table, l'UI afficherait une case « trames en retard » sur une source
***REMOVED*** d'ENTRÉE où rien n'est mesuré — exactement l'erreur déjà commise (et corrigée) pour le loudness
***REMOVED*** et l'ANC : une case affichée là où rien n'est sondé. `alarmes_par_slot` et l'API filtrent
***REMOVED*** dessus ; l'UI (settings.html) fait de même pour la colonne.
ALARMES_ROLE = {
    "rx": ("frozen", "black", "silence", "clip", "gamut"),
    "tx": ("frozen", "black", "silence", "clip", "gamut", "tx_late"),
}

***REMOVED*** QUOI EN FAIRE, par source. On choisit un NIVEAU, pas des canaux : le service d'alertes route déjà
***REMOVED*** par seuil (`alerting_min_level`, surchargeable par canal — mail à partir d'« erreur », webhook à
***REMOVED*** partir d'« avertissement »…). Cocher des destinations ici créerait une SECONDE vérité sur le
***REMOVED*** routage, à côté de celle-là, avec la question insoluble de qui l'emporte quand elles divergent —
***REMOVED*** et une colonne de plus à chaque canal ajouté. Le niveau, lui, compose avec l'existant et vaudra
***REMOVED*** aussi pour les canaux futurs.
***REMOVED***   log     : journal technique seulement — n'entre même pas dans le fil d'alertes
***REMOVED***   info    : dans le fil, sous le seuil de notification par défaut → aucune notification
***REMOVED***   warning : comportement historique (défaut)
***REMOVED***   error   : remonté à tous les canaux
...
def normalize(flows):
    ...
    for f in (flows or []):
        ...
        if al:
            item["alarmes"] = al        ***REMOVED*** absent = défauts (on ne persiste pas du bruit)
        niv = str(f.get("niveau") or "").lower()
        if niv in NIVEAUX and niv != NIVEAU_DEFAUT:
            item["niveau"] = niv        ***REMOVED*** idem : on ne persiste que ce qui dévie du défaut
        out.append(item)
    return out
...
def alarmes_par_slot(dc_params, role="rx"):
    ...
    for f in active_flows(dc_params, role):
        ***REMOVED*** UNIQUEMENT les flux VIDÉO — et ce n'est pas un raccourci. Le moteur sonde l'image ET le son
        ***REMOVED*** d'un slot, puis publie les deux dans le signal du slot VIDÉO (`sig.update(vres)` puis
        ***REMOVED*** `sig.update(sres)` dans controller.py) ; côté orchestrateur, `_check_signal` n'est appelé
        ***REMOVED*** que pour les receivers vidéo. Le silence de `_audio_3` remonte donc sur `Rx ***REMOVED***3`, et l'ANC
        ***REMOVED*** n'a AUCUNE sonde. Produire des entrées pour ("audio", n) / ("anc", n) donnerait des clés
        ***REMOVED*** que personne ne consulte, et une UI qui promet des réglages sans effet.
        if f.get("essence") != "video":
            continue
        niv = str(f.get("niveau") or "").lower()
        ***REMOVED*** Résolu PUIS restreint aux drapeaux du rôle (cf. ALARMES_ROLE) : un flux RX persisté avec
        ***REMOVED*** un vieux `tx_late` (bascule de rôle, éditeur bas niveau…) ne doit pas ressusciter côté UI.
...
def grouping_maps(flows):
    ...
    """
    flows = flows or []
    vid_of = {}   ***REMOVED*** (essence, idx) → video idx
    sub_of = {}   ***REMOVED*** (essence, idx) → rang dans la vidéo (par essence)
    counters = {}  ***REMOVED*** (video_idx|None, essence) → compteur
    for f in flows:
        if f.get("essence") == "video":
            continue
...
***REMOVED*** ── Accès unifié : flux stockés (Option A) OU dérivés du legacy (repli transparent) ───────────
...
def render_groups(flows):
    ...
    ***REMOVED*** rang (sub) 0-based par essence dans chaque groupe vidéo
    for g in groups:
        for i, a in enumerate(g["audios"]):
            a["sub"] = i
        for i, d in enumerate(g["ancs"]):
            d["sub"] = i
    ***REMOVED*** flux indépendants → chacun son groupe (au bout)
