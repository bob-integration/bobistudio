...
RX_REPAIR_TOUS_LES_N_POLLS = 12   # ~1 min entre deux reprises (tick de surveillance = 5 s)
RX_REPAIR_MAX = 5                 # au plus 5 reprises par épisode, puis on laisse l'alerte parler
...
_rx_repair_en_vol = set() # vmid dont une tentative de reprise est en cours (jamais deux)
_rx_repair_n      = {}    # vmid → nb de reprises tentées pour CET épisode
...
def _check_sessions_moteur(vmid, hn, receivers):
    ...
    if actives < attendus:
        ...
        # ★ ON RÉPARE, on ne se contente pas de le dire. Les tentatives sont espacées
        # (RX_REPAIR_TOUS_LES_N_POLLS ≈ 1 min) et BORNÉES par épisode : une cause qui n'est pas
        # une perte d'abonnement — source amont éteinte, lien coupé — ne doit pas faire marteler
        # le contrôleur indéfiniment. Au-delà, l'alerte reste, et c'est à un humain de voir.
        ...
        _rx_repair_n.pop(vmid, None)      # épisode clos : le compteur repart pour le suivant
