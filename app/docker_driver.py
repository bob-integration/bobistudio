...
def resync_moteur(vmid, params):
    ...
    # 3) VÉRIFICATION, puis REPRISE JUSQU'À UNE ÉCHÉANCE.
    #
    # ⚠ CE BLOC FAISAIT EXACTEMENT DEUX TENTATIVES, au motif écrit ici qu'« au-delà, c'est une
    # panne, pas une course ». Le 2026-09-12 ce raisonnement a coûté 13 MINUTES D'ANTENNE MUETTE :
    # moteur redémarré à 09:42:21, repush à +7 s puis à +39 s — tous deux dans le vide, le moteur
    # n'ayant pas fini `mtl_init` —, alerte posée, et PLUS AUCUNE TENTATIVE. Les flux ne sont
    # revenus qu'après un `repush_subscriptions` lancé à la main.
    #
    # Le défaut n'est pas le nombre d'essais, c'est de compter des ESSAIS au lieu de tenir une
    # ÉCHÉANCE. Deux essais de 30 s bornent la reprise à ~60 s, alors que la docstring de cette
    # même fonction dit que le moteur met « 30-60 s de plus » que `:8081/status` à être prêt : le
    # budget pouvait expirer avant que le moteur ne soit en état d'écouter. On tient donc une
    # échéance franche, et on continue de re-pousser tant qu'elle n'est pas atteinte.
    ...
    while True:
        ...
        # On re-pousse à CHAQUE tour : l'opération est idempotente, et le cas qui nous intéresse
        # est justement celui où le moteur n'était pas prêt à écouter au tour précédent.
        n_push += 1
        log.warning("resync moteur %s: %s/%d sessions RX — repush #%d",
                    vmid, obs, attendus, n_push)
        try:
            _nmos.repush_subscriptions(vmid)
        except Exception:
...
# Intervalle entre deux tentatives de reprise, et ÉCHÉANCE TOTALE de la reprise. 5 minutes
# couvrent largement les 30-60 s de `mtl_init` sur E810 (entraînement du lien 100G compris) et
# une nouvelle recréation qui s'enchaînerait. Au-delà, ce n'est plus une course : on alerte.
_VERIF_RX_PAS_S = 10
...
def derive_config_moteur(vmid, payload=None):
    ...
    for r in (db_get_node_interfaces(c.get("node_id")) or []):
        if (r.get("role") or "") != "media2110":
            continue
        ifn = r.get("ifname")
        # `ip_cidr` porte le masque, le moteur publie l'adresse seule : on compare les adresses.
