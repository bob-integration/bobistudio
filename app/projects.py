...
***REMOVED*** Types exclus des projets (infra persistante/partagée, jamais clonée par projet).
***REMOVED*** storage = filebrowser scopé au projet, recréé manuellement, fichiers sur le host.
***REMOVED*** 2110_io (décision 2026-07-05) : le moteur ST 2110 est LIÉ AU NŒUD (un par nœud),
***REMOVED*** jamais à un projet — ni créable dans un projet, ni snapshoté dedans. Les projets
***REMOVED*** le consomment via les PORTS ; sa config se sauvegarde par les config-snapshots
***REMOVED*** par-container (POST /api/containers/<vmid>/config_snapshots).
PROJECT_EXCLUDED_TYPES = {"webrtc_gateway", "storage", "2110_io"}

***REMOVED*** ─── Cycle de vie (chantier 3) : verrou par projet ────────────
***REMOVED*** Le modèle threading n'a pas de lock par-VMID ; on verrouille au niveau PROJET :
***REMOVED*** un seul chargement/déchargement à la fois par projet (anti double-restore).
_project_locks = {}
...
***REMOVED*** ─── « Projet vivant » : re-snapshot auto débouncé (chantier 3) ─────
***REMOVED*** Toute modif de deploy_config d'un container appartenant à un projet re-snapshote le
***REMOVED*** projet après REsnap_DEBOUNCE_S sans nouvelle modif : décharger ne perd jamais rien.
***REMOVED*** L'ancien snapshot est poussé dans project_versions (version auto, rétention bornée).

RESNAP_DEBOUNCE_S = 10
_resnap_timers = {}          ***REMOVED*** pid → threading.Timer
...
def _resnapshot_safe(pid):
    ...
    try:
        ...
        log.warning("re-snapshot auto projet ***REMOVED***%s: %s", pid, e)
...
def resnapshot_projet(pid, label=None, force=False):
    ...
    if (p.get("state") or "saved") in ("loading", "unloading"):
        return None   ***REMOVED*** la prochaine modif re-déclenchera
    ...
    if not vmids:
        return None   ***REMOVED*** projet déchargé : on garde le dernier snapshot tel quel
    ...
    if not force and json.dumps(snapshot, sort_keys=True) == json.dumps(old, sort_keys=True):
        if label:   ***REMOVED*** rien n'a changé mais on veut un point nommé
            db_add_project_version(pid, old, label=label)
        return None
    db_add_project_version(pid, old, label=label)
    db_update_project_snapshot(pid, snapshot)
    log.info("projet ***REMOVED***%s re-snapshoté (%d containers%s)", pid, len(snapshot),
...
def _ensure_media_dir(p):
    ...
    try:
        ...
        log.warning("media dir projet ***REMOVED***%s (%s): %s", p.get("id"), path, e)
    db_set_project_media_path(p["id"], path)
    p["media_path"] = path
    return path


***REMOVED*** Champs multicast des tx_slots (2110_io) : JAMAIS rejoués d'un snapshot — purgés au
***REMOVED*** chargement pour forcer la réallocation via mcast_allocations dans le hook
***REMOVED*** before_deploy (sinon deux chargements du même export émettent sur les mêmes groupes,
***REMOVED*** conflit silencieux). Les sessions RX (adresses de sources EXTERNES) ne sont pas touchées.
...
def _prefix_snapshot(snapshot, prefix):
    ...
    def pref(name):                       ***REMOVED*** nom de shm « nu »
        return f"{prefix}-{name}" if name and name in produced else name

    def pref_path(path):                  ***REMOVED*** « /dev/shm/X » ou « X »
        if not path:
            return path
    ...
    for c in snapshot:
        ...
        if dc:
            ...
            t = dc.get("type")
            ***REMOVED*** Sorties explicites (les sorties dérivées du hostname suivent le nouveau nom)
            for k in ("shm_out", "shm_out_pgm", "shm_out_clean", "shm_out_pvw"):
                if params.get(k):
                    params[k] = pref(params[k])
            ***REMOVED*** Entrées (références internes → préfixées ; externes → inchangées)
...
def verifier_capacite(snapshot):
    ...
    ***REMOVED*** 1) Cœurs CPU (agrégé cluster : le nœud cible est choisi au vol par pick_compute_node).
    ***REMOVED***    Neutralisé si AUCUN nœud n'a de pool de pinning (compute_cpuset) : dans ce mode,
    ***REMOVED***    allocate_cores retombe sur le quota --cpus sans pinning, il n'y a rien à épuiser.
    try:
        from .core_pool import cores_status
        stats = [cores_status(n["id"]) or {} for n in nodes]
        pool_total = sum(s.get("total", 0) for s in stats)
        if pool_total > 0:
            ***REMOVED*** ★ La demande vient du coût MESURÉ du type quand on en a un (p95 sur 4 h glissantes,
            ***REMOVED*** cf. app/cpu_profiles.py), et seulement à défaut du `cores` déclaré. Comparer du
            ***REMOVED*** déclaré à du déclaré ne valide rien : les deux côtés de l'inégalité étaient des
            ***REMOVED*** chiffres que personne n'avait jamais vérifiés. `mesures` dit, par type, d'où vient
            ***REMOVED*** le nombre retenu — un refus de déploiement doit pouvoir s'expliquer.
            ...
            for c in snapshot:
                ...
                if t in ("2110_io",):
                    continue
                ***REMOVED*** Ordre de préséance, et il compte : la MESURE d'abord (seul chiffre vérifié) ;
                ***REMOVED*** à défaut le `cores` du snapshot, qui est l'intention de CETTE instance (un
                ***REMOVED*** override explicite doit être respecté) ; le manifeste seulement en dernier.
                ***REMOVED*** Ainsi, tant qu'aucune mesure n'existe, le pré-vol se comporte EXACTEMENT comme
                ***REMOVED*** avant — on ne déplace pas un seuil de refus par effet de bord.
            ...
            need = int(need + 0.999) if need else 0        ***REMOVED*** arrondi SUPÉRIEUR : un pré-vol se trompe
            free = sum(s.get("free", 0) for s in stats)    ***REMOVED*** du côté prudent, jamais de l'autre
            if need > free:
    ...
    ***REMOVED*** 2) (retiré 2026-07-05) — le 2110_io est exclu des projets (PROJECT_EXCLUDED_TYPES,
    ***REMOVED***    lié au nœud) : le restore le SAUTE, plus de conflit « un moteur par nœud » ici.

    ***REMOVED*** 3) Multicast : les adresses TX du snapshot seront RÉALLOUÉES au chargement —
    ***REMOVED***    il faut assez d'adresses libres dans les plages.
    try:
        import ipaddress
        ...
        for c in snapshot:
            ...
            ***REMOVED*** copie profonde légère : on compte sans muter le snapshot
...
def planifier_restore(snapshot):
    ...
    remaps = []        ***REMOVED*** [{from, to, hostname}]
    host_conflicts = []
    shm_conflicts  = []
    ***REMOVED*** On remappe à la volée pour éviter qu'un VMID alloué pendant le plan
    ***REMOVED*** ne soit re-choisi pour un autre container du même snapshot.
    ...
    for c in snapshot:
        ...
        new = old
        if old in reserved:
            ***REMOVED*** Allocation MONOTONE (handle local) : prochain au-dessus de tous les réservés. Jamais
            ***REMOVED*** d'échec (plus de plafond). Le vmid source est de toute façon sans valeur sur ce cluster.
...
def _attendre_agent(ip, timeout=30, vmid=None):
    ***REMOVED*** `vmid` sert UNIQUEMENT à joindre l'en-tête d'auth de l'agent (X-MXL-Agent-Token) : depuis que
    ***REMOVED*** MXL_AGENT_TOKEN est réellement injecté au `docker run`, un agent sous token répond 401 sans
    ***REMOVED*** l'en-tête → la restauration voyait « agent injoignable (timeout) » et SAUTAIT le déploiement
    ***REMOVED*** de chaque conteneur restauré.
    from . import deploy   ***REMOVED*** helper mTLS :8081 (repli http si CA absente)
    debut = time.time()
    while time.time() - debut < timeout:
        try:
...
def restaurer_projet(project_id, progress=None, should_abort=None, only_vmids=None, preserve_uuid=False):
    ...
    ***REMOVED*** Verrou de cycle de vie (chantier 3) : un seul chargement à la fois par projet.
...
def _restaurer_projet_verrouille(project_id, p, _p, _abort, only_vmids, preserve_uuid):
    ...
    ***REMOVED*** Dossier média garanti (un projet importé/cloné n'en a pas → les containers
    ***REMOVED*** retomberaient sur la racine partagée de tous les projets).
    _ensure_media_dir(p)

    ***REMOVED*** Receivers d'abord (les multiviews ont besoin de leur SHM), puis workers
    ...
    ***REMOVED*** Préfixe le hostname par le nom du projet + réécrit les références shm internes.
    snapshot = _prefix_snapshot(snapshot, p["name"])

    ***REMOVED*** Reprise ciblée des échecs : on ne garde que les VMID d'origine demandés.
    ...
    if not plan["can_restore"]:
        ...
        ***REMOVED*** Un seul message composé n'est pas traduisible terme à terme (chaque raison est une
        ***REMOVED*** sous-phrase française) : une alerte PAR raison bloquante, chacune une clé complète.
        if plan["hostname_conflicts"]:
        ...
        if plan.get("error"):
            ***REMOVED*** Site mort en pratique (`planifier_restore` ne pose jamais "error") : conservé par
            ***REMOVED*** défense, clé générique sans contenu français composé.
    ...
    total     = len(snapshot)
    ok_count  = 0
    failed    = []   ***REMOVED*** [{vmid (origine), hostname, reason}]
    vmid_map  = {}   ***REMOVED*** vmid d'origine (snapshot) → vmid créé (remap des vues à la fin)
    ...
    for i, c in enumerate(snapshot, 1):
        ...
        def _echec(reason, niveau="error", clef=None, **extra):
            ***REMOVED*** `reason` reste le texte LIBRE consommé tel quel par l'UI (failed[].reason,
            ***REMOVED*** static/scripts.js) — inchangé. `clef` (optionnel) est la clé i18n migrée pour
            ***REMOVED*** l'alerte elle-même : jurisprudence oblige (jamais un verdict/texte libre en
            ***REMOVED*** paramètre), CHAQUE site d'appel a sa propre clé complète plutôt qu'un {reason}
            ***REMOVED*** générique.
            if clef:
        ...
        ***REMOVED*** Types liés au nœud / infra partagée (2110_io, passerelle…) : jamais restaurés
        ***REMOVED*** par projet — présents dans les VIEUX snapshots seulement, sautés sans erreur.
        if _dc_type in PROJECT_EXCLUDED_TYPES:
            _p(f"  · {hostname} : type « {_dc_type} » lié au nœud/infra — non restauré par projet (ignoré)")
            total -= 1
            continue

        ***REMOVED*** 1) Création : chemin Docker (compute/MTL) uniquement ; un snapshot non-docker est rejeté.
        _p("      · clonage + configuration + démarrage…")
        from . import plugins as _pl
        _is_docker = bool(_dc_type) and _pl.runtime(_dc_type) == "docker"
        if _is_docker:
            ***REMOVED*** Type docker-only (chemin compute) : pas de vaisseau LXC. On crée le conteneur
            ***REMOVED*** compute sur son nœud (épinglé dans le snapshot, sinon auto-pick) ; deployer_script
            ***REMOVED*** fera le docker run + attente agent + push du script (cf. _insert_udc).
            ...
            if not _vmid_new:
                ...
                continue
            vmid = _vmid_new   ***REMOVED*** vmid synthétique alloué par le chemin compute
        else:
            ***REMOVED*** Backend LXC/Proxmox retiré (full-Docker) : un snapshot d'un type non-docker
            ***REMOVED*** (ancien conteneur LXC) n'a plus de cible de restauration. Rejet explicite.
        ...
        ***REMOVED*** Identité d'instance : COPIE = nouvel uuid (généré à la création, défaut, sûr) ;
        ***REMOVED*** DÉPLACEMENT (preserve_uuid) = on réapplique l'instance_uuid du snapshot → identité/NMOS
        ***REMOVED*** conservées (cf. doublon si la source tourne encore — assumé par le choix opérateur).
        if preserve_uuid and c.get("instance_uuid"):
            db_set_instance_uuid(vmid, c["instance_uuid"])
            _p("      · identité conservée (déplacement)")

        ***REMOVED*** Mémoires par-container (DVE, multiview, presets…) : recréées sous le
        ***REMOVED*** nouveau vmid (scope remappé). Embarquées dans le snapshot à la sauvegarde.
        ...
        ***REMOVED*** Multicast TX : JAMAIS rejoué du snapshot — purge → le hook before_deploy
        ***REMOVED*** réalloue via mcast_allocations (owner_ref = nouveau vmid).
        params = dc.get("params", {}) or {}
        n_mc = _purge_tx_multicast(params)
        if n_mc:
            _p(f"      · {n_mc} adresse(s) multicast TX à réallouer au déploiement")

        ***REMOVED*** Chemin docker-only : pas d'attente IP/agent LXC ; deployer_script fait le docker run +
        ***REMOVED*** attente agent + push script. On rattache le projet AVANT (le bind média lit project_id).
        if _is_docker:
        ...
        ***REMOVED*** 4) Déploiement (protégé : un échec ne doit plus tuer toute la boucle)
    ...
    ***REMOVED*** Remap des VUES (chantier 3) : les widgets référencent instance_uuid/vmid — après un
    ***REMOVED*** rechargement (les originaux n'existent plus), on re-pointe les widgets orphelins sur
    ***REMOVED*** les clones. Une COPIE à côté d'originaux vivants ne vole pas les vues (on ne remappe
    ***REMOVED*** que si l'ancienne cible a disparu).
    if vmid_map:
        try:
            _remap_project_views(project_id, vmid_map, _p)
        except Exception as e:
            log.warning("remap vues projet ***REMOVED***%s: %s", project_id, e)
    ...
    if ok_count:
        ***REMOVED*** La production TOURNE : c'est ici qu'elle reçoit son niveau de tally, pas à
        ***REMOVED*** l'enregistrement. Un serveur qui stocke cinquante projets et n'en joue qu'un n'a pas à
        ***REMOVED*** traîner cinquante niveaux que rien ne sert. Idempotent : un projet rejoué retrouve le
        ***REMOVED*** sien, avec son UUID — ce que ses conteneurs citent.
        try:
...
def detruire_containers_projet(project_id):
    ...
    try:
        db_add_alert("alert.projet.destruction_en_cours", "warning", kind="deploy",
                     params={"p": p["name"]})
        ***REMOVED*** Cibles = containers RATTACHÉS au projet (project_id — couvre les vmids remappés
        ***REMOVED*** au chargement) + les vmids du snapshot encore vivants (rattachement legacy).
