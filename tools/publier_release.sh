...
# ── Les notes viennent du CHANGELOG ──────────────────────────────────────────
#
# ★ CE QUI A ÉTÉ PERDU, ET RETROUVÉ ICI. Jusqu'à la 0.9.5, les releases portaient le RÉCIT des
# nouveautés — ce que le lecteur vient chercher. Les 0.9.6 et 0.9.7, publiées avec ce script,
# n'ont eu que son texte générique : un mode d'emploi d'installation à la place du contenu. On
# ne recopie donc plus rien à la main : la section la plus récente de CHANGELOG.md EST la note.
# Le mode d'emploi passe en pied, où il ne prend la place de rien.
...
try:
    ...
    sys.exit(0)
# La première section de niveau 2, jusqu'au séparateur ou à la section suivante.
m = re.search(r"^## .+?$(.*?)(?=^---\s*$|^## )", texte, re.S | re.M)
if not m:
    sys.exit(0)
titre = re.search(r"^## (.+?)$", texte, re.M).group(1).strip()
print("## " + titre + "\n" + m.group(1).strip())
PYEOF
)"
[ -n "$NOTES_CORPS" ] || warn "CHANGELOG.md illisible ou sans section — notes réduites au pied de page."

# ★ CE TEXTE DIT CE QUI SERT À QUOI, ET DANS QUEL ORDRE.
# La version précédente enchaînait « le paquet ci-joint est l'artefact d'installation complet »
# et « l'archive Source code ne permet pas d'installer » : un lecteur en concluait que le zip
# était NÉCESSAIRE pour installer. Il ne l'est pas — `get.sh` récupère le dépôt principal puis
# chaque sous-module à la version épinglée, avec `curl` seul. La confusion a été signalée par un
# lecteur de la 0.9.7 ; elle vient de deux vraies qui se lisaient comme une fausse.
# On énonce donc d'abord le chemin NORMAL, puis à quoi sert le zip, puis l'avertissement.
...
## À quoi sert le paquet ci-joint
...
## ⚠ N'utilisez pas l'archive « Source code » de GitHub
