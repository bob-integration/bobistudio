...
# Mot de passe root : le preseed versionné ne porte qu'un MARQUEUR, jamais une valeur. On en tire un
# au hasard par ISO construite, on le substitue ici, et on l'affiche une seule fois à la fin. Il ne
# sert qu'à la console physique ou iLO : l'exploitation passe par la clé SSH injectée plus bas.
# Alphabet sans caractères ambigus (ni O/0, ni l/1/I) — il se tape à la main, souvent en QWERTY et
# en pleine panne. Même alphabet dans app/pxe.py (PRESEED_MDP_ALPHABET).
...
[ ${#ROOT_PASSWORD} -eq 20 ] || die "tirage du mot de passe root incomplet"
sed -i "s#$MARQUEUR_MDP#$ROOT_PASSWORD#g" "$PRESEED_USE"
