...
def anc_payload_maison(frame):
    ...
    udw = bytes(range(16))                         ***REMOVED*** charge utile bidon, repérable
    meta = struct.pack("<HHHHHHHH", 0x60, 0x60, len(udw), 9, 0, 0, 0, 0)  ***REMOVED*** 16 o
    return struct.pack("<II", 1, len(udw)) + meta + udw


t0 = time.time()
k = 0
n = SR // 100                                       ***REMOVED*** blocs de 10 ms
t = np.arange(n, dtype=np.float32) / SR
while time.time() - t0 < DURATION:
    ***REMOVED*** Audio : un sinus par canal (440 Hz × (ch+1)) → valeurs vérifiables côté stock.
    blk = np.stack([np.sin(2 * np.pi * 440 * (c + 1) * (t + k * n / SR)) * 0.5
                    for c in range(CH)], axis=1).astype(np.float32)
    wa.write(blk)
    if k % 4 == 0:                                  ***REMOVED*** ANC à 25 fps
