...
***REMOVED***
***REMOVED*** probe_2110 — PLACEHOLDER de script.
***REMOVED***
***REMOVED*** La sonde ST 2110 est un type MATÉRIEL MTL (plugin.json: needs_dpdk=true) : son runtime réel
***REMOVED*** est le CONTRÔLEUR baké dans l'image `bobi-mtl` (docker/controller.py + binaire mtl_rx), lancé
***REMOVED*** par `app/docker_driver.deploy_docker` — PAS ce fichier. `deploy_docker` construit la ligne
***REMOVED*** `docker run` (env RX-only + TIMING_PARSER=1 + PF vfio dédiée) et ne rend JAMAIS ce script.py
***REMOVED*** (le chemin `render_script`/agent :8081/deploy est réservé aux types compute/LXC). Le parser de
***REMOVED*** conformité 2110-21 vit déjà dans libmtl (mtl_rx.c, cf. docs/reference/PROBE_2110.md) : la sonde le WRAPPE.
***REMOVED***
***REMOVED*** Ce fichier n'existe que pour satisfaire le registre de plugins (`plugins._scan` exige un
***REMOVED*** `script_template` valide + dry-run `.format`). Il fournit un repli minimal — un `:8080` qui
***REMOVED*** annonce le mode placeholder — au cas TRÈS improbable où il serait déployé hors du chemin MTL.
***REMOVED*** Toute accolade littérale est DOUBLÉE (contrat str.format des plugins).
